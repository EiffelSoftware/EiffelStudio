			...
			if l_members /= Void then
				...
				loop
					...
					if l_method /= Void then
						a_type.add_member (create {CODE_MEMBER_REFERENCE}.make (l_method.name, a_type, l_method.attributes & feature {SYSTEM_DLL_MEMBER_ATTRIBUTES}.Override = feature {SYSTEM_DLL_MEMBER_ATTRIBUTES}.Override))
					else
						l_property ?= l_member
