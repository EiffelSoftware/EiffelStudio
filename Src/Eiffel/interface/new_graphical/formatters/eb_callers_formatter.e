	...
	mode: NATURAL_8
		...
		do
			...
			when {DEPEND_UNIT}.is_in_assignment_flag then
				Result := {ES_FEATURE_RELATION_TOOL_VIEW_MODES}.assigners
			when {DEPEND_UNIT}.is_in_creation_flag then
				Result := {ES_FEATURE_RELATION_TOOL_VIEW_MODES}.creators
			else
		...
		do
			...
			if Result = Void then
				inspect
					flag
				when {DEPEND_UNIT}.is_in_assignment_flag then
					create Result.make_filled (pixmaps.icon_pixmaps.feature_assigners_icon, 1, 2)
				when {DEPEND_UNIT}.is_in_creation_flag then
					create Result.make_filled (pixmaps.icon_pixmaps.feature_creators_icon, 1, 2)
				else
					create Result.make_filled (pixmaps.icon_pixmaps.feature_callers_icon, 1, 2)
				end
