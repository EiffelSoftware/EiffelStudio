...
feature {NONE} -- Implementation
	...
	generated_routine_name: STRING is
		...
		local
			l_this_target_object: CODE_THIS_REFERENCE_EXPRESSION
		do
			if target_object /= Void then
	...
	caller_type: TYPE is
		...
		local
			this_target_object: CODE_THIS_REFERENCE_EXPRESSION
			argument_target_object: CODE_ARGUMENT_REFERENCE_EXPRESSION
			variable_target_object: CODE_VARIABLE_REFERENCE_EXPRESSION
			type_target_object: CODE_TYPE_REFERENCE_EXPRESSION
			...
			if internal_caller_type /= Void then
				...
				if this_target_object /= Void then
					...
					if l_parent_name = Void then
						Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_implementing_type, [current_class, routine_name])
					else
					...
					if argument_target_object /= Void then
						...
						if type_target_object /= Void then
							...
							if variable_target_object /= Void then
								...
								Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_target_object, ["routine reference expression"])
							end
						end
					end
				end
	...
	implementing_type (a_dotnet_type_name: STRING; a_dotnet_feature_name: STRING; feature_arguments: LINKED_LIST [CODE_EXPRESSION]): STRING is
		...
		local
			...
			l_parents: LINKED_LIST [CODE_PARENT]
		do
			if Resolver.is_generated_type (a_dotnet_type_name) then
				...
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Non_generated_type, [a_dotnet_type_name, "implementing_type", "routine reference expression"])
			end			
		end
