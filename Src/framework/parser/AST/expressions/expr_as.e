...
feature -- Conversion
	...
	is_detachable_expression: BOOLEAN
			-- Does current represent an expression that might be detachable?
			--| Useful for finding out if `exp' in `attached exp ...' is valid
			--| for an object test.
		do
				-- By default False. Redefined in descendants that might be detachable
		end
