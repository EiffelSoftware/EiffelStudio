indexing
	description: "Object to generate class generic domains used in Eiffel query language"
...
feature -- Access
	...
	criterion: QL_GENERIC_CRITERION
			-- Criterion used when generating new items

	domain: QL_GENERIC_DOMAIN is
			-- Generated domain
		do
...
feature -- Visit
	...
	process_class (a_item: QL_CLASS) is
			-- Process `a_item'.
		local
			l_gen_list: EIFFEL_LIST [FORMAL_DEC_AS]
			l_generic: QL_GENERIC
		do
			if a_item.is_compiled then
				l_gen_list := a_item.class_c.generics
				if l_gen_list /= Void and then not l_gen_list.is_empty then
					from
						l_gen_list.start
					until
						l_gen_list.after
					loop
						create l_generic.make_with_ast (l_gen_list.item, a_item)
						evaluate_item (l_generic)
						l_gen_list.forth
					end
				end
			end
...
feature{NONE} -- Implementation
	...
	tautology_criterion: like criterion is
		...
		do
			Result :=
				generic_criterion_factory.simple_criterion_with_index (generic_criterion_factory.c_is_compiled) and
				generic_criterion_factory.simple_criterion_with_index (generic_criterion_factory.c_true)
		end

	compiled_criterion: like criterion is
			-- A criterion that only compiled items can satisfy
		do
			Result := generic_criterion_factory.simple_criterion_with_index (generic_criterion_factory.c_is_compiled)
		end
