deferred class CORBA_ATTRIBUTEDEF
...
feature { COURIER, CONSUMER }
    ...
    consume (msg : ANY; idl_type : INTEGER) is 
        ...
        local
            ...
            x6  : CORBA_TYPECODE
            x7  : CORBA_IDLTYPE
            x8  : CORBA_ATTRIBUTEMODE
            x9  : CORBA_CONTAINER
            x10  : STRING
            x11  : STRING
            x12  : CORBA_IDLTYPE
            x13  : CORBA_ATTRIBUTEMODE
        ...
        do
            ...
            when 17 then
                x8 := mode
                ca.make1 (corba_attributemode_tc)
                x8.enum_to_any (ca)
            when 18 then
                add_in_arg_with_type ("val", corba_attributemode_tc)
                dum := req.set_params (nvl)
                create x13.make_default
                x13.enum_from_any (nvl.get_any_value_by_name ("val"))
                set_mode (x13)
                x13.enum_to_any (nvl.get_any_value_by_name ("val"))
            end
            finish_invoke
        end
