...
class
	ECDP_TYPE_CONVERTER_EIFFEL_TO_DOTNET
...
feature -- Implementation
	...
	dotnet_type_name (an_eiffel_type_name: STRING): STRING is
		...
		do
			...
			elseif Known_eiffel_types.has (an_eiffel_type_name) then
				...
				l_referenced_assemblies := (create {ECDP_REFERENCED_ASSEMBLIES}).referenced_assemblies
				from
				...
				loop
					...
					if l_cat = Void then
						l_cat := local_cache_reflection.assembly_types (l_referenced_assemblies.item.assembly.get_name)
					end
					check
						non_void_l_cat: l_cat /= Void
					end
					from
						l_counter := 1
					until
	...
	dotnet_type_expression (an_expression: SYSTEM_DLL_CODE_EXPRESSION): STRING is
		...
		local
			...
			l_method_ref_exp: SYSTEM_DLL_CODE_METHOD_REFERENCE_EXPRESSION
	...
	consumed_type (a_dotnet_type_name: STRING): CONSUMED_TYPE is
		...
		do
			...
			if l_cache_reflection.Types_cache.found_item /= Void then
				...
				if Result = Void then
					create local_cache_reflection.make_with_path (local_cache_path, Clr_version)
					l_ca := local_cache_reflection.local_consumed_assemblies
					from
						l_counter := 1
					until
						l_counter > l_ca.count or Result /= Void
	...
	consumed_type_from_dotnet_type_name (ca: CONSUMED_ASSEMBLY; dotnet_type_target_feature: STRING): CONSUMED_TYPE is
		...
		do
			...
			if Result = Void then
				...
				Result := l_local_cache_reflection.consumed_type_from_dotnet_type_name (ca, dotnet_type_target_feature)
			end
		end
...
end -- Class ECDP_TYPE_CONVERTER_EIFFEL_TO_DOTNET