...
feature -- Access
	...
	feature_names: LINKED_LIST [STRING]
			-- | ARRAY_LIST [STRING]
		indexing
			description: "List of exported features names"
		end
		
	exportation_list: LINKED_LIST [STRING]
			-- | ARRAY_LIST [STRING]
		indexing
...
feature -- Status Setting
	...
	set_exportation_list (a_list: like exportation_list) is
 	...
 	set_all is
 		...
 		require
 			no_feature_name: feature_names.count = 0
 		do
...
feature -- Basic Operations
	...
	add_feature_name (a_feature_name: STRING) is
		...
		require
			...
			not_empty_feature_name: a_feature_name.count > 0
		do
			if not feature_names.has (a_feature_name) then
				feature_names.extend (a_feature_name)
			end
		ensure
	...
	string_representation: STRING is
		...
		do
			if exportation_list /= Void and then exportation_list.count > 0 then
				...
				loop
					a_class_name ?= exportation_list.i_th (i)
					if a_class_name /= Void and then a_class_name.count > 0 then
				...
				Result.append(Closing_curl_bracket)
				Result.append(Space)
				Result.append(intern_string_representation)
			else
				Result.append(intern_string_representation)
			end
		end
