...
feature {CODE_CONSUMER_FACTORY} -- Visitor features.
	...
	generate_property (a_source: SYSTEM_DLL_CODE_MEMBER_PROPERTY) is
		...
		require
			non_void_source: a_source /= Void
		local
			l_property_getter: CODE_PROPERTY_GETTER
			l_property_setter: CODE_PROPERTY_SETTER
		do
...
feature {NONE} -- Routine Initialization
	...
	initialize_creation_routine (a_source: SYSTEM_DLL_CODE_CONSTRUCTOR; a_creation_routine: CODE_CREATION_ROUTINE) is
		...
		do
			if current_type = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_type, ["creation routine initialization (" + context (a_creation_routine) + ")"])
			else
	...
	initialize_root_procedure (a_source: SYSTEM_DLL_CODE_MEMBER_METHOD; a_root_procedure: CODE_ROOT_PROCEDURE) is
			-- Generate Eiffel code from `a_source'.
		...
		do
			if current_type = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_type, ["root creation routine initialization (" + context (a_root_procedure) + ")"])
			else
				...
				if a_root_procedure.name = Void then
					Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_feature_name, ["root creation routine initialization (" + context (a_root_procedure) + ")"])
				else
	...
	initialize_procedure (a_source: SYSTEM_DLL_CODE_MEMBER_METHOD; a_procedure: CODE_PROCEDURE) is
			-- Generate Eiffel code from `a_source'.
			-- | Set `type_feature' with `Basic_operation' 
			-- | Add `a_creation_routine' to `current_type'
			-- | Call `initialize_routine'.
		...
		do
			if current_type = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_type, ["root creation routine initialization (" + context (a_procedure) + ")"])
			else
	...
	initialize_property_getter (a_source: SYSTEM_DLL_CODE_MEMBER_PROPERTY; a_property_getter: CODE_PROPERTY_GETTER) is
		...
		do
			if current_type = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_type, ["property getter initialization (" + context (a_property_getter) + ")"])
			else
	...
	initialize_property_setter (a_source: SYSTEM_DLL_CODE_MEMBER_PROPERTY; a_property_setter: CODE_PROPERTY_SETTER) is
		...
		do
			if current_type = Void then
				...
				if a_source.type = Void then
					Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_return_type, ["property setter initialization"])
				else
	...
	initialise_function (a_source: SYSTEM_DLL_CODE_MEMBER_METHOD; a_function: CODE_FUNCTION) is
			-- Generate Eiffel code from `a_source'.
		...
		do
			if current_type = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_type, ["routine initialization"])
			else
	...
	initialize_routine (a_source: SYSTEM_DLL_CODE_MEMBER_METHOD; a_routine: CODE_ROUTINE) is
			-- Generate Eiffel code from `a_source'.
...
feature {NONE} -- Implementation
	...
	generate_arguments (a_source: SYSTEM_DLL_CODE_TYPE_MEMBER; a_routine: CODE_ROUTINE) is
			-- Generate Eiffel code from `a_source'.
		...
		do
			...
			if l_method = Void then
				...
				if l_property = Void then
					Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Failed_assignment_attempt, ["SYSTEM_DLL_CODE_TYPE_MEMBER", "SYSTEM_DLL_CODE_MEMBER_METHOD or SYSTEM_DLL_CODE_MEMBER_PROPERTY", "statements generation of " + context (a_routine)])
				else
				...
				loop
					...
					if l_argument /= Void then
						...
						if l_argument_expression /= Void then
							...
							Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Failed_assignment_attempt, ["CODE_EXPRESSION", "CODE_ARGUMENT_EXPRESSION", "arguments generation of " + context (a_routine)])
						end
					else
						Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Void_argument, [context (a_routine)])
					end
					i := i + 1
				end
			else
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_parameters, [context (a_routine)])
			end
		end	

	generate_result (a_source: SYSTEM_DLL_CODE_TYPE_MEMBER; a_function: CODE_FUNCTION) is
			-- Generate Eiffel code from `a_source'.
		...
		do
			...
			if l_method = Void then
				l_property ?= a_source
				if l_property = Void then
					Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Failed_assignment_attempt, ["CODE_EXPRESSION", "CODE_ARGUMENT_EXPRESSION", "result generation of " + context (a_function)])
				else
			...
			if l_return_type = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_return_type, [context (a_function)])
			else
	...
	generate_statements (a_statements: SYSTEM_DLL_CODE_STATEMENT_COLLECTION; a_routine: CODE_ROUTINE) is
		...
		local
			...
			l_try_catch_statement: CODE_TRY_CATCH_FINALLY_STATEMENT
		do
			if a_statements = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_statements, [context (a_routine)])
			else
				from
				...
				loop
					...
					if l_statement = Void then
						Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Void_statement, [context (a_routine)])
					else
	...
	initialize_try_catch_feature (a_try_catch_statement: CODE_TRY_CATCH_FINALLY_STATEMENT; a_routine: CODE_ROUTINE) is
		...
		local
			...
			l_rescue_feature: CODE_ROUTINE
			l_snippet_value: STRING
			l_snippet_statement: CODE_SNIPPET_STATEMENT
		...
		do
			if current_routine = Void then
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_routine, ["try catch feature initialization (" + context (a_routine) + ")"])
			else
