...
feature {FUNC_EDITOR} -- Function Editor
...
feature -- Input and Output stones 
	...
	set_lists (i: like input_list; o: like output_list) is
			-- Set `input_list' to `i'.
			-- Set `output_list' to `o'.
		do
			set (i, o);
			if
				not (func_editor = Void)
			then
				func_editor.set_lists (input_list, output_list);
			end
		end;
...
feature -- List operations
	...
	go_i_th (i: INTEGER) is
		...
		do
			...
			if not (func_editor = Void) then
				func_editor.go_i_th (i);
			end;
		end;
