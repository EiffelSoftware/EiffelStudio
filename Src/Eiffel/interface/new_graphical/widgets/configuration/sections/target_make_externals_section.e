...
feature -- Access
	...
	icon: EV_PIXMAP is
			-- Icon of the section.
		once
			Result := pixmaps.icon_pixmaps.project_settings_make_file_icon
		end
...
feature -- Element update
	...
	add_external is
			-- Add a new external.
		local
			l_external: CONF_EXTERNAL_MAKE
			l_ext_sec: EXTERNAL_MAKE_SECTION
		do
				-- add it in the configuration
			l_external := configuration_window.conf_factory.new_external_make ("new")
			target.add_external_make (l_external)
...
feature {NONE} -- Implementation
	...
	context_menu: ARRAYED_LIST [EV_MENU_ITEM] is
		...
		do
			...
			create l_item.make_with_text_and_action (conf_interface_names.external_add_make, agent add_external)
	...
	update_toolbar_sensitivity is
		...
		do
			...
			toolbar.add_make_button.select_actions.extend (agent add_external)
