...
-----------------------------------------------------------*/
{
	...
	hr = p_IDocHostUIHandler->ShowContextMenu(tmp_dw_id,ppt,pcmdt_reserved,pdisp_reserved);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IDocHostUIHandler->GetHostInfo(p_info);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IDocHostUIHandler_impl_proxy::ccom_resize_border(  /* [in] */ ecom_control_library::tagRECT * prc_border,  /* [in] */ ecom_control_library::IOleInPlaceUIWindow * p_uiwindow,  /* [in] */ EIF_INTEGER f_rame_window )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IDocHostUIHandler->ResizeBorder(prc_border,p_uiwindow,tmp_f_rame_window);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IDocHostUIHandler->IDocHostUIHandler_TranslateAccelerator(lpmsg,pguid_cmd_group,tmp_n_cmd_id);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IDocHostUIHandler_impl_proxy::ccom_get_drop_target(  /* [in] */ ecom_control_library::IDropTarget * p_drop_target,  /* [out] */ EIF_OBJECT pp_drop_target )
...
-----------------------------------------------------------*/
{
	...
	SHORT * tmp_pch_urlin = 0;
	tmp_pch_urlin = (SHORT *)rt_ec.ccom_ec_pointed_short (eif_access (pch_urlin), NULL);
	SHORT * * tmp_ppch_urlout = 0;
	tmp_ppch_urlout = (SHORT * *)grt_ec_control_interfaces2.ccom_ec_pointed_cell_462 (eif_access (ppch_urlout), NULL);
	...
	grt_ce_control_interfaces2.ccom_ce_pointed_cell_462 ((SHORT * *)tmp_ppch_urlout, ppch_urlout);
	
	grt_ce_control_interfaces2.ccom_free_memory_pointed_460 (tmp_pch_urlin);
grt_ce_control_interfaces2.ccom_free_memory_pointed_462 (tmp_ppch_urlout);

};
/*----------------------------------------------------------------------------------------------------------------------*/

void ecom_control_library::IDocHostUIHandler_impl_proxy::ccom_filter_data_object(  /* [in] */ ecom_control_library::IDataObject * p_do,  /* [out] */ EIF_OBJECT pp_doret )
...
-----------------------------------------------------------*/
{
	...
	ecom_control_library::IDataObject * * tmp_pp_doret = 0;
	tmp_pp_doret = (ecom_control_library::IDataObject * *)grt_ec_control_interfaces2.ccom_ec_pointed_cell_246 (eif_access (pp_doret), NULL);
	...
	grt_ce_control_interfaces2.ccom_ce_pointed_cell_246 ((ecom_control_library::IDataObject * *)tmp_pp_doret, pp_doret);
