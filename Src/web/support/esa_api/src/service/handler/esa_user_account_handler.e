	...
	do_get (req: WSF_REQUEST; res: WSF_RESPONSE)
		...
		do
			...
			if attached current_media_type (req) as l_type then
				...
				l_rhf.new_representation_handler (esa_config, l_type, media_type_variants (req)).account_information_page (req, res)
			else
				log.write_information (generator+".do_get Processing request not acceptable")
				l_rhf.new_representation_handler (esa_config, "", media_type_variants (req)).account_information_page (req, res)
			end
		end
