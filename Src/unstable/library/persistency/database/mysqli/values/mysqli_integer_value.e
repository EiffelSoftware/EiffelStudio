note
	description: "MySQL INTEGER Value"
	author: "haroth@student.ethz.ch"
	date: "$Date$"
	revision: "$Revision$"

class
	MYSQLI_INTEGER_VALUE
...
feature -- Access (Date and Time Types)
	...
	as_datetime: DATE_TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_INTEGER_VALUE.as_datetime: current 'value' is not representable as a datetime.")
		end

	as_year: INTEGER
			-- This value as a INTEGER
		do
			raise ("MYSQLI_INTEGER_VALUE.as_year: current 'value' is not representable as a year.")
		end
...
feature -- Access (Set Type)
	...
	as_set: SET [STRING]
		...
		do
			...
			Result := l_linked_set
			raise ("MYSQLI_INTEGER_VALUE.as_set: current 'value' is not representable as a set.")
		end
