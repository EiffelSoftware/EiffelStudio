	...
	execute_safe (a_routine: ROUTINE [ANY, TUPLE])
		...
		do
			if not l_rescued then
				...
				if attached {FUNCTION [ANY, detachable TUPLE, detachable ANY]} a_routine as l_func then
					if attached {FUNCTION [ANY, detachable TUPLE, BOOLEAN]} l_func as l_bool_func then
						last_boolean := l_bool_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, CHARACTER_8]} l_func as l_char8_func then
						last_character_8 := l_char8_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, CHARACTER_32]} l_func as l_char32_func then
						last_character_32 := l_char32_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, INTEGER_8]} l_func as l_int8_func then
						last_integer_8 := l_int8_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, INTEGER_16]} l_func as l_int16_func then
						last_integer_16 := l_int16_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, INTEGER_32]} l_func as l_int32_func then
						last_integer_32 := l_int32_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, INTEGER_64]} l_func as l_int64_func then
						last_integer_64 := l_int64_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, NATURAL_8]} l_func as l_nat8_func then
						last_natural_8 := l_nat8_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, NATURAL_16]} l_func as l_nat16_func then
						last_natural_16 := l_nat16_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, NATURAL_32]} l_func as l_nat32_func then
						last_natural_32 := l_nat32_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, NATURAL_64]} l_func as l_nat64_func then
						last_natural_64 := l_nat64_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, REAL_32]} l_func as l_real32_func then
						last_real_32 := l_real32_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, REAL_64]} l_func as l_real64_func then
						last_real_64 := l_real64_func.last_result
					elseif attached {FUNCTION [ANY, detachable TUPLE, POINTER]} l_func as l_pointer_func then
