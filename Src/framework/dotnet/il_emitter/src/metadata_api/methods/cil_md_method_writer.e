...
feature -- Method body definition
	...
	new_method_body (token: INTEGER): CIL_MD_METHOD_BODY
			-- Get a new body for method token `token'.
		require
			not_closed: not is_closed
			last_one_has_been_emitted: is_previous_body_written
			is_method_token:
				token & {CIL_MD_TOKEN_TYPES}.Md_mask = {CIL_MD_TOKEN_TYPES}.Md_method_def
		do
...
feature -- Update
	...
	update_rvas (md_emit: CIL_MD_METADATA_EMIT; top_rva: INTEGER)
...
feature -- Settings
	...
	write_current_body
		...
		local
			...
			l_m: like item
			l_ex: CIL_MD_EXCEPTION_CLAUSE
			is_fat_seh: BOOLEAN
			i: INTEGER
			l_old_exceptions: detachable ARRAY [CIL_MD_EXCEPTION_CATCH]
		...
		do
			...
			check l_meth /= Void then
				...
				update_size (l_pos + l_meth_size + {CIL_MD_FAT_METHOD_HEADER}.Count)
				...
				then
					...
					if l_meth.has_exceptions_handling then
						Fat_method_header.set_flags ({CIL_MD_METHOD_CONSTANTS}.More_sections)
					end
...
feature {NONE} -- Implementation
	...
	internal_method_body: detachable CIL_MD_METHOD_BODY
...
feature {NONE} -- constants
	...
	tiny_method_header: CIL_MD_TINY_METHOD_HEADER
			-- To avoid over creating method headers.
		once
			create Result.make (0)
		end

	fat_method_header: CIL_MD_FAT_METHOD_HEADER
			-- To avoid over creating method headers.
		once
			create Result.make (0, 0, 0)
		end

	exception_header: CIL_MD_METHOD_SECTION_HEADER
