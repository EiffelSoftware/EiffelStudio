...
feature -- Settings
	...
	set_byref_type (element_type: INTEGER_8; a_token: INTEGER)
		...
		require
			token_valid: (element_type = {CIL_MD_SIGNATURE_CONSTANTS}.Element_type_class or
				element_type = {CIL_MD_SIGNATURE_CONSTANTS}.Element_type_valuetype) implies
				a_token /= 0
		do
			internal_put ({CIL_MD_SIGNATURE_CONSTANTS}.Element_type_byref, current_position)
...
feature {NONE} -- Implementation
	...
	compress_type_token (i: INTEGER)
		...
		do
			...
			if l_header = {CIL_MD_TOKEN_TYPES}.Md_type_ref then
					-- TypeRef token
				l_encoding := 1
			elseif l_header = {CIL_MD_TOKEN_TYPES}.Md_type_def then
					-- TypeDef token
				l_encoding := 0
			elseif l_header = {CIL_MD_TOKEN_TYPES}.Md_type_spec then
					-- TypeSpec token
				l_encoding := 2
			else
				check
