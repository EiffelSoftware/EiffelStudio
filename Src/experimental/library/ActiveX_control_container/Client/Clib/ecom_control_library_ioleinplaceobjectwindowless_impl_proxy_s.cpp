...
ecom_control_library::IOleInPlaceObjectWindowless_impl_proxy::IOleInPlaceObjectWindowless_impl_proxy( IUnknown * a_pointer )
{
  ...
  p_IOleInPlaceObjectWindowless = NULL;
...
-----------------------------------------------------------*/
{
  ...
  hr = p_IOleInPlaceObjectWindowless->SetObjectRects((const struct tagRECT *)lprc_pos_rect, (const struct tagRECT *)lprc_clip_rect);
  if (FAILED (hr))
  {
    if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
      com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
