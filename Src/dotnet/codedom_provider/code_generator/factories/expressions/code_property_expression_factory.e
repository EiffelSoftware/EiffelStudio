...
feature {CODE_CONSUMER_FACTORY} -- Visitor features.
	...
	generate_property_reference_expression (a_source: SYSTEM_DLL_CODE_PROPERTY_REFERENCE_EXPRESSION) is
		...
		local
			a_property_reference_expression: CODE_PROPERTY_REFERENCE_EXPRESSION
		do
	...
	generate_property_set_value_reference_expression (a_source: SYSTEM_DLL_CODE_PROPERTY_SET_VALUE_REFERENCE_EXPRESSION) is
		...
		require
			non_void_source: a_source /= Void
		local
			a_property_set_value_reference_expression: CODE_PROPERTY_SET_VALUE_REFERENCE_EXPRESSION
		do
...
feature {NONE} -- Implementation
	...
	initialize_property_reference_expression (a_source: SYSTEM_DLL_CODE_PROPERTY_REFERENCE_EXPRESSION; a_property_reference_expression: CODE_PROPERTY_REFERENCE_EXPRESSION) is
		...
		do
			...
			if current_namespace /= Void then
				...
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_namespace, ["property reference expression"])
			end
			if current_type /= Void then
				a_property_reference_expression.set_current_class (current_type.name)
			else
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_current_type, ["property reference expression"])
			end
		ensure
	...
	initialize_property_set_value_reference_expression (a_source: SYSTEM_DLL_CODE_PROPERTY_SET_VALUE_REFERENCE_EXPRESSION;
															a_property_set_value_reference_expression: CODE_PROPERTY_SET_VALUE_REFERENCE_EXPRESSION) is
			-- Generate Eiffel code from `a_source'.
		...
		do
			Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Does_nothing, ["property set value reference expression"])
		end
