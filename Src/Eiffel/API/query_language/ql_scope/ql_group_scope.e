note
	description: "Object that represents a group scope"
...
feature{QL_SHARED_SCOPES} -- Initialization
	...
	make
			-- Initialize.
		do
			index := group_scope_index
		ensure
			scope_index_set: index = group_scope_index
		end

feature -- Metric support

	basic_scope (a_calculate_function: FUNCTION [ANY, TUPLE [QL_GROUP], DOUBLE];
	                  a_criterion: QL_GROUP_CRITERION): QL_METRIC_GROUP_BASIC_SCOPE_INFO
...
feature -- Access
	...
	path_domain_generator (a_item: QL_GROUP; a_path: READABLE_STRING_GENERAL): QL_DOMAIN_GENERATOR
			-- Domain generator for current scope
		local
		...
		do
			...
			elseif assertion_path_marker.is_equipped_with_marker (a_path) then
				...
				end			elseif argument_path_marker.is_equipped_with_marker (a_path) then
			...
			elseif
				group_path_marker.is_equipped_with_marker (a_path) or
				target_path_marker.is_equipped_with_marker (a_path)
			then
				if a_item.group.is_library then
					create {QL_TARGET_DOMAIN_GENERATOR}Result
					Result.set_criterion (create {QL_TARGET_NAME_IS_CRI}.make_with_setting (target_path_marker.base_name (a_path), False, {QL_NAME_CRITERION}.identity_matching_strategy))
				else
	...
	empty_domain: QL_GROUP_DOMAIN
			-- An empty domain whose scope is group
		do
			create Result.make
		end

	delayed_domain: QL_DELAYED_GROUP_DOMAIN
