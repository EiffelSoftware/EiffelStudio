...
feature -- Access: file name
	...
	Eiffel_recent_projects: STRING is
		...
		once
			if Platform_constants.is_windows then
				Result := "HKEY_CURRENT_USER\Software\ISE\Eiffel50\recent projects"
			else
	...
	Eiffel_preferences: STRING is
		...
		once
			if Platform_constants.is_windows then
				Result := "HKEY_CURRENT_USER\Software\ISE\Eiffel50"
			else
				create fname.make_from_string (Execution_environment.home_directory_name)
	...
	last_opened_projects_resource_name: STRING is "bench_recent_files"
...
feature -- Access: command name
	...
	Ebench_command_name: FILE_NAME is
			-- Complete path to `ebench'.
		once
			create Result.make_from_string (Eiffel_installation_dir_name)
			Result.extend_from_array (<<"bench", "spec", Eiffel_platform, "bin">>)
			Result.set_file_name ("ebench")
		end
