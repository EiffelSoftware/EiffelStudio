...
#include "ecom_eiffel_compiler_IEnumCluster_impl_stub_s.h"
...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_Eif_compiler.ccom_ce_pointed_cell_28 (rgelt, NULL));
		if (*rgelt != NULL)
			(*rgelt)->AddRef ();
	}
	...
	if (*rgelt != NULL)
		(*rgelt)->Release ();
	grt_ec_Eif_compiler.ccom_ec_pointed_cell_28 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_Eif_compiler.ccom_ce_pointed_cell_24 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_Eif_compiler.ccom_ec_pointed_cell_24 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
