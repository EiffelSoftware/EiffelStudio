...
class
	ECDP_DELEGATE_EXPRESSION_FACTORY
...
feature {ECDP_CONSUMER_FACTORY} -- Visitor features.
	...
	generate_delegate_create_expression (a_source: SYSTEM_DLL_CODE_DELEGATE_CREATE_EXPRESSION) is
		...
		local
			a_delegate_create_expression: ECDP_DELEGATE_CREATE_EXPRESSION
		do
	...
	generate_delegate_invoke_expression (a_source: SYSTEM_DLL_CODE_DELEGATE_INVOKE_EXPRESSION) is
		...
		require
			non_void_source: a_source /= Void
		local
			a_delegate_invoke_expression: ECDP_DELEGATE_INVOKE_EXPRESSION
		do
...
feature {NONE} -- Implementation
	...
	initialize_delegate_create_expression (a_source: SYSTEM_DLL_CODE_DELEGATE_CREATE_EXPRESSION; a_delegate_create_expression: ECDP_DELEGATE_CREATE_EXPRESSION) is
		...
		do
			...
			if l_delegate_type /= Void then
				...
				(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Missing_delegate_type, ["delegate create expression"])
			end
			...
			if l_target_object /= Void then
				...
				(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Missing_target_object, ["delegate create expression"])
			end
			a_delegate_create_expression.set_method_name (a_source.method_name)
		end

	initialize_delegate_invoke_expression (a_source: SYSTEM_DLL_CODE_DELEGATE_INVOKE_EXPRESSION; a_delegate_invoke_expression: ECDP_DELEGATE_INVOKE_EXPRESSION) is
			-- | Call `generate_expression_from_dom' to generate `target_object'.
		...
		do
			...
			if a_target_object /= Void then
				...
				(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Missing_target_object, ["delegate invoke expression"])
			end
		end

	generate_delegate_arguments (a_source: SYSTEM_DLL_CODE_DELEGATE_INVOKE_EXPRESSION; delegate_invoke_expression: ECDP_DELEGATE_INVOKE_EXPRESSION) is
		...
		do
			...
			if l_arguments /= Void then
				...
				(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Missing_parameters, ["delegate invoke expression"])
			end
		end

end -- class ECDP_DELEGATE_EXPRESSION_FACTORY
