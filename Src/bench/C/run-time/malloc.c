...
#ifdef MEMPANIC
#define mempanic	panic("memory inconsistency")
#else
...
rt_private char *malloc_free_list(unsigned int nbytes, union overhead **hlist, int gc_flag)
{
	 ...
	 * there is no inconsistency of we cannot allocate from the free list after
...
rt_private char *allocate_free_list(register unsigned int nbytes, register union overhead **hlist)
{
	...
	for (; i < NBLOCKS; i++) {
		if ((selected = hlist[i]) == (union overhead *) 0)
			...
			/* Walk trhough list, until we find a good block. This
                			...
                			/* Eiffel object or memory chunk */
{
	 ...
	 * a free can put the block back into the right free list. Note that an
...
rt_public char *xrealloc(register char *ptr, register unsigned int nbytes, int gc_flag)
{
	...
	if (zone->ov_flags & EO_SPEC) {
		...
		dprintf(16)("realloc: special has count = %d, elemsize = %d\n",
			*pointer, *(pointer + 1));
...
rt_public void sc_stop(void)
{
	...
	xfree(sc_to.sc_arena);				/* This one is completely free */
...
rt_private void inspect();	/* %%ss undefine */
