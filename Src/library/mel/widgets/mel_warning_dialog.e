...
feature {NONE} -- Initialization
	...
	create_widget (p_so: POINTER; w_name: ANY; auto_manage_flag: BOOLEAN) is
			-- Create a warning dialog with `auto_manage_flag'.
		do
			if auto_manage_flag then
				screen_object := xm_create_warning_dialog (p_so, $w_name, default_pointer, 0)
			else
				screen_object := xm_create_warning_dialog (p_so, $w_name, auto_unmanage_arg, 1)
			end;
		end;

feature {NONE} -- Implementation

	xm_create_warning_dialog (a_parent, a_name, arglist: POINTER; argcount: INTEGER): POINTER is
		external
