...
#include "ecom_EiffelComCompiler_IEiffelSystemExternals_impl_stub.h"
...
-----------------------------------------------------------*/
{
	...
	if (tmp_value != NULL)
	{
		...
		*pp_ienum_include_paths = grt_ec_ISE_c.ccom_ec_pointed_interface_218 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
...
-----------------------------------------------------------*/
{
	...
	{
		EIF_OBJECT tmp_object = eif_protect (tmp_value);
		*pp_ienum_object_files = grt_ec_ISE_c.ccom_ec_pointed_interface_221 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
