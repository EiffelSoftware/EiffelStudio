...
if enabled_debug_trace then
	print("Debuggables.item (body_id):%N");
	print("===========================%N");
	print(sent_debuggables.item (body_id));
	print("%N%N%N%N");
end
				...
				loop
if enabled_debug_trace then
	print("%Nbreakables.item.position: ");
	print(breakables.item.position);
end
...
feature {EWB_REQUEST}
	...
	set_all_breakables is 
		...
		do
if enabled_debug_trace then
	print ("Setting all Breakable Points for all debugged routines.%N")
end;
			from
				debugged_routines.start
			until
			...
			loop
if enabled_debug_trace then
	print ("Setting routine_breakables for feature:%N");
	print (debugged_routines.item.name);
	print (" (");
	print (debugged_routines.item.id);
	print (")%N");
end;
...
if enabled_debug_trace then
	print ("Finished setting all Breakable Points for all debugged routines.%N")
end
		end;
			...
			if not once_debuggables.has (f.body_id) then
				...
				loop
					...
					loop
						...
						bp.set_real_body_id	(r_body_id);
if enabled_debug_trace then
	print ("%TExtending new breakpoints with breakable point for:%N");
	print ("%T%Toffset: ");
	print (breakable_points.item.position);
	print ("%N")
end;
						new_breakpoints.extend (bp);
						breakable_points.forth
