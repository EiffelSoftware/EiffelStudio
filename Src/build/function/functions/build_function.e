...
feature -- Insertion and Delections
	...
	add_element_line (i: INTEGER; input_d, output_d: PND_DATA) is
	...
	remove_element_line (elt: like input_data; is_command: BOOLEAN) is
		...
		do
			...
			if not input_list.after then
				if is_command then
					create cut_elt_line_cmd
					create arg.make (Current)
					cut_elt_line_cmd.execute (arg, Void)
				end
				remove
			end
		end
...
feature {NONE}
	...
	input_list_put (elt: PND_DATA) is
		do
			input_list.put (elt)			
		end

	output_list_put (elt: PND_DATA) is
		do
			output_list.put (elt)			
		end
	...
	has_data (l: EB_LINKED_LIST [PND_DATA]; s: PND_DATA): BOOLEAN is
		...
		do
			...
			then
				...
				if not il.empty then
					create merge_command
					merge_command.set_old_lists (input_list, output_list)
					input_list.merge (il)
					output_list.merge (ol)
					create arg.make (Current)
					merge_command.execute (arg, Void)
				end
			end
		end
