...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleCache->Cache(p_formatetc,tmp_advf,tmp_pdw_connection);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IOleCache_impl_proxy::ccom_init_cache(  /* [in] */ ecom_control_library::IDataObject * p_data_object )
...
void ecom_control_library::IOleCache_impl_proxy::ccom_set_data(  /* [in] */ ecom_control_library::tagFORMATETC * p_formatetc,  /* [in] */ EIF_OBJECT pmedium,  /* [in] */ EIF_INTEGER f_release )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleCache->SetData(p_formatetc,tmp_pmedium,tmp_f_release);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
