	...
	display_half_selected(d_y: INTEGER; start_selection, end_selection: INTEGER; a_device: EV_PIXMAP) is
		...
		do
			...
			if start_selection /= 1 then
				local_position := display_blanks(local_position, d_y, a_device, False, 1, start_selection-1)
			end

				-- Display the 'selected' area
			local_position := display_blanks(local_position, d_y, a_device, True, start_selection, end_selection-1)

				-- if the selection do not end at the end of the token,
				-- Display the last 'non selected' area
			if end_selection <= length then
				local_position := display_blanks(local_position, d_y, a_device, False, end_selection,length)
			end
		end
