...
/*
doc:	<struct name="processor" export="shared">
...
doc:		<field name="wait_condition_subscribers" type="struct subscriber_list_t (a vector of processor*)"> The list of clients that registered for a wait condition change signal. </field>
...
doc:		<field name="generated_private_queues" type="struct private_queue_list_t (a vector of priv_queue*)">
...
doc:			* For passive processors, it would make sense to split processor into two structs region and processor.
...
doc:	</struct>
*/
class processor /*TODO: Rename as rt_processor? */
{
public:
