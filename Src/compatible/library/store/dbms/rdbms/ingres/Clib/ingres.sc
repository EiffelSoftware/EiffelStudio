...
int c_ing_make (int m_size)
{
  ...
  EXEC SQL SET_SQL (ERRORHANDLER = ing_err_handler);
...
int ing_exec_immediate (char *order)
{
	...
	EXEC SQL EXECUTE IMMEDIATE :dbCommand;
...
int ing_init_order (int no_des, char *order)
{
	...
	ing_tranNumber = 1;
	ing_clear_error();
	EXEC SQL PREPARE :stateName INTO :dap FROM :dbCommand;
	...
	if (GetVarNum(dap) < colNum) {
	  if (colNum > IISQ_MAX_COLS) {
	    ...
	    EXEC SQL DESCRIBE :stateName USING DESCRIPTOR :dap;
	  }
	}
    if (error_number) {
		free(dap);
	...
	/*otherwise,  it is not   a select statement */
		EXEC SQL DECLARE :cursorName CURSOR FOR :stateName;
	}
     ...
     /* return 0 if there is a next row, 1 if no row left */
{
	...
	EXEC SQL FETCH :cursorName USING DESCRIPTOR :dap;
...
int ing_connect (char *name, char *passwd, char *role, char *rolePassWd, char *group, char *baseName)
{
	...
	if ((group!=NULL) && (strlen(group)>0))  {
		if ((role != NULL) && (strlen(role)>0)) {
			...
			dbVariable = name;
			dbGroup = tmp;
			EXEC SQL CONNECT :dbName IDENTIFIED BY :dbVariable OPTIONS = :dbCommand,:dbGroup;
		}
		else {
			sprintf(cmd, "-G%s", group);
			dbCommand = cmd;
			dbVariable = name;
			EXEC SQL CONNECT :dbName IDENTIFIED BY :dbVariable OPTIONS = :dbCommand;
		}
	}
	else {
		if ((role != NULL) && (strlen(role)>0)) {
			...
			dbCommand = name;
			EXEC SQL CONNECT :dbName IDENTIFIED BY :dbCommand;
		}
	}
  	return error_number;
}
...
int ing_get_year(void)
{
	...
	EXEC SQL SELECT  date_part('year',date(:dateString)) INTO :year;
	EXEC SQL BEGIN;
	EXEC SQL END;
  	return year;
}
...
int ing_get_month(void)
{
	...
	EXEC SQL SELECT  date_part('month',date(:dateString)) INTO :month;
	EXEC SQL BEGIN;
	EXEC SQL END;
	if (month == 0)
		month = 1;
...
int ing_get_day(void)
{
	...
	EXEC SQL SELECT  date_part('day',date(:dateString)) INTO :day;
	EXEC SQL BEGIN;
	EXEC SQL END;
	if (day == 0)
		day = 1;
...
int ing_get_hour(void)
{
	EXEC SQL BEGIN DECLARE SECTION;
		int hour; 
	EXEC SQL END   DECLARE SECTION;
	EXEC SQL SELECT  date_part('hour',date(:dateString)) INTO :hour;
	EXEC SQL BEGIN;
	EXEC SQL END;
  	return hour;
}
...
int ing_get_min(void)
{
	EXEC SQL BEGIN DECLARE SECTION;
		int minute;  
	EXEC SQL END   DECLARE SECTION;
	EXEC SQL SELECT  date_part('minute',date(:dateString)) INTO :minute;
	EXEC SQL BEGIN;
	EXEC SQL END;
  	return minute;
}
...
int ing_get_sec(void)
{
	EXEC SQL BEGIN DECLARE SECTION;
		int second;
	EXEC SQL END   DECLARE SECTION;
	EXEC SQL SELECT  date_part('sec',date(:dateString)) INTO :second;
	EXEC SQL BEGIN;
	EXEC SQL END;
  	return second;
}
...
{
	EXEC SQL INQUIRE_SQL(:error_number = ERRORNO, :error_message = ERRORTEXT);
#ifdef TEST_INGRES
	printf("===============================  ERROR#: %d  ===============================\n",error_number);
	printf("MESSAGE:%s\n",error_message);
	printf("WARNING:%s\n",warn_message);
	 ...
	 for(i=0; i<dap->sqld; i++) {
		...
		case IISQ_TXT_TYPE:
			...
			if (dap->sqlvar[i].sqllen == DB_OUT_OF_INGRES_LIMIT) {
				strcpy(date_Tyc,  dap->sqlvar[i].sqldata);
				EXEC SQL  select  date_part('year',date(:date_Tyc)) INTO :year;
				EXEC SQL  select  date_part('month',date(:date_Tyc)) INTO :month;
				EXEC SQL  select  date_part('day',date(:date_Tyc)) INTO :day ;
				EXEC SQL  select  date_part('hour',date(:date_Tyc)) INTO :hour;
				EXEC SQL  select  date_part('minute',date(:date_Tyc)) INTO :minute;
				EXEC SQL  select  date_part('sec',date(:date_Tyc)) INTO :sec ;
				EXEC SQL BEGIN;
				EXEC SQL END;
