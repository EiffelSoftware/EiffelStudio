...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_183 (rgelt, NULL));
		if (*rgelt != NULL)
			(*rgelt)->AddRef ();
	}
	...
	if (*rgelt != NULL)
		(*rgelt)->Release ();
	grt_ec_ISE_c.ccom_ec_pointed_cell_183 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
