...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_ISE.ccom_ce_pointed_cell_192 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_ISE.ccom_ec_pointed_cell_192 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
