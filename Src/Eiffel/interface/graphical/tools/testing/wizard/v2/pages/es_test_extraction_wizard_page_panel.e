...
feature {NONE} -- Initialization
	...
	make (a_composition: like composition; a_stack_list: like call_stack_elements)
			-- Initialize `Current'.
		require
			a_stack_list_empty: a_stack_list.is_empty
		do
			call_stack_elements := a_stack_list
	...
	initialize_with_session (a_service: SESSION_MANAGER_S)
		...
		do
			...
			if debugger_manager.application_is_executing and then debugger_manager.application_is_stopped then
				initialize_call_stack (stack_frame_count.value.as_natural_32)
			end
			on_valid_state_change
		end

	initialize_call_stack (a_stack_frame_count: NATURAL)
			-- Initialize call stack in `grid'.
		local
		...
		do
			...
			if l_stack /= Void then
				...
				loop
					...
					populate_row (l_row, l_cse, a_stack_frame_count)
					l_stack.forth
					i := i + 1
				end
			end
...
feature {NONE} -- Access
	...
	stack_frame_count: EV_SPIN_BUTTON
			-- Text field containg stack frame count
...
feature {NONE} -- Internationalization
	...
	call_stack_text: STRING = "Stack frames to extract"
	stack_frames_text: STRING = "Number of extracted stack frames selected by default"
