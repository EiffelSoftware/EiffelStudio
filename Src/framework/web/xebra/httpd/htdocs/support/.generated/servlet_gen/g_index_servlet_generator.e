...
feature-- Implementation
	...
	get_root_tag: XTAG_TAG_SERIALIZER
			--<Precursor>
		local
			root_tag, temp: XTAG_TAG_SERIALIZER
			stack: ARRAYED_STACK [XTAG_TAG_SERIALIZER]
		do
			create stack.make (10)
				-- line: 6 row: 8 of file: ./index.xeb
			create {XTAG_XEB_CONTAINER_TAG} temp.make
			temp.debug_information := "line: 6 row: 8 of file: ./index.xeb"
			root_tag := temp
			...
			temp.current_controller_id := "controller_1"
			temp.tag_id := "content"
				-- line: 2 row: 43 of file: ./index.xeb
			create {XTAG_PAGE_NOOP_TAG} temp.make
			temp.debug_information := "line: 2 row: 43 of file: ./index.xeb"
			stack.item.add_to_body (temp)
			temp.put_value_attribute("class", "MAIN_CONTROLLER")
			temp.current_controller_id := "controller_1"
			...
			temp.current_controller_id := "controller_1"
			temp.tag_id := "content"
				-- line: 5 row: 8 of file: ./index.xeb
			create {XTAG_XEB_HTML_TAG} temp.make
			temp.debug_information := "line: 5 row: 8 of file: ./index.xeb"
			stack.item.add_to_body (temp)
			temp.current_controller_id := "controller_1"
			temp.tag_id := "body"
			stack.put (temp)
			...
			stack.item.add_to_body (temp)
			temp.put_value_attribute("text", "%Nsupport works!%N")
			temp.current_controller_id := "controller_1"
			temp.tag_id := "content"
			stack.remove
