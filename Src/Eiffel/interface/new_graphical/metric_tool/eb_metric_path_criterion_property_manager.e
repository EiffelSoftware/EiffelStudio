indexing
	description: "Property manager for path criterion"
...
class
	EB_METRIC_PATH_CRITERION_PROPERTY_MANAGER
...
feature -- Access
	...
	property_item: EV_GRID_EDITABLE_ITEM is
...
feature -- Properties management
	...
	load_properties (a_criterion: like criterion_type) is
		...
		do
			property_item.set_text (a_criterion.path)
		end

	store_properties (a_criterion: like criterion_type) is
			-- Store properties in current manager into `a_criterion'.
		do
			a_criterion.set_path (property_item.text.out)
		end

feature{NONE} -- Implementation

	criterion_type: EB_METRIC_PATH_CRITERION
			-- Anchor type
