	...
	feature_type (a_name: STRING): STRING is
		...
		do
			...
			if Features.found then
				...
				Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Feature_name_not_found, [a_name])
			end
		end
	...
	add_feature (a_type_name: STRING; an_eiffel_feature_name: STRING) is
		...
		do
			if not is_generated_type (a_type_name) then
				features.put (a_type_name, (create {CODE_ENTITY_NAME_RESOLVER}).unique_entity_name (an_eiffel_feature_name))
			else
				features.put (a_type_name, an_eiffel_feature_name)
			end
		end

	initialize_features (a_type: CODE_GENERATED_TYPE) is
		...
		local
			...
			l_get_property: CODE_PROPERTY_GETTER
			l_set_property: CODE_PROPERTY_SETTER
		...
		do
			...
			if l_external_type /= Void then
				...
				if l_consumed_type /= Void then
					...
					Event_manager.raise_event (feature {CODE_EVENTS_IDS}.Missing_consumed_type, [an_external_type_name])
				end
			end
		end
