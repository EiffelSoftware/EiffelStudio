	...
	make is
		...
		do
			...
			if not is_successful then
				last_error_message := Warning_messages.w_Cannot_read_file (Eiffel_Ace.file_name) +
...
feature -- Access
	...
	root_class_name: STRING is
		...
		local
			res: STRING
		do
			create res.make_from_string (root_ast.root.root_name)
			res.to_upper
			Result := res
		ensure
			valid_ast: is_valid
	...
	clusters: LINKED_LIST [STRING] is
...
feature -- Element change
	...
	add_cluster (name, parent_name, path: STRING) is
		...
		do
			if parent_name = Void or parent_name.is_empty then
...
feature -- Status report
	...
	is_valid: BOOLEAN is
		...
		do
			Result := False
			if Workbench.system_defined or else Eiffel_ace.file_name /= Void then
				tmp_ast := Eiffel_ace.Lace.parsed_ast
	...
	apply is
		...
		do
			...
			if not ace_file.exists or else ace_file.is_writable then
				...
				last_error_message := Warning_messages.W_not_creatable (Eiffel_ace.file_name)
			end
