...
feature -- Basic Operations
	...
	raise_event (an_id: INTEGER; a_context: TUPLE) is
		...
		local
			l_events: ECDP_EVENTS
			l_event: ECDP_EVENT
		do
			create l_events
			if not l_events.is_event_id (an_id) then
				raise_event (feature {ECDP_EVENTS_IDS}.Invalid_event_identifier, [an_id.out])
			elseif not l_events.is_valid_context (an_id, a_context) then
				raise_event (feature {ECDP_EVENTS_IDS}.Invalid_event_context, [l_events.event_name (an_id), l_events.event_arguments_count (an_id).out, a_context.count.out])
			else
	...
	process_exception is
		...
		do
			raise_event (feature {ECDP_EVENTS_IDS}.Rescued_exception, [feature {ISE_RUNTIME}.last_exception])
		end

feature {NONE} -- Implementation

	log (an_event: ECDP_EVENT) is
