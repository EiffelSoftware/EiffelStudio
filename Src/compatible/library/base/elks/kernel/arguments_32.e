...
feature {NONE} -- Implementation
	...
	option_word_equal (arg, w: READABLE_STRING_GENERAL): BOOLEAN
		...
		do
			...
			elseif not arg.is_empty and then arg.code (1).to_character_32 = option_sign.item then
	...
	option_word_begins_with (arg, w: READABLE_STRING_GENERAL): BOOLEAN
		...
		do
			...
			elseif arg.code (1).to_character_32 = option_sign.item and then arg.count > w.count then
	...
	option_character_equal (arg: READABLE_STRING_GENERAL; c: CHARACTER_32): BOOLEAN
		...
		do
			if option_sign.item = '%U' then
				Result := arg.has_code (c.natural_32_code)
			elseif arg.code (1).to_character_32 = option_sign.item then
				Result := arg.substring (2, arg.count).has_code (c.natural_32_code)
			end
		end
