		...
		do
			...
			loop
				...
				if c = escape_character.natural_32_code then
					...
					if i < n and then s.code (i + 1).to_character_32 = escape_character then
		...
		do
			...
			loop
				...
				if c = escape_character.natural_32_code then
						-- We might be facing a character that was escaped.
					if i < n then
						if s.code (i + 1).to_character_32 = escape_character then
								-- The `escape_character' was escaped, it meant they really wanted an `escape_character'.
							i := i + 1
						elseif s.code (i + 1).to_character_32 = 'u' then
							if i + 4 < n then
