...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=C:\\Build-OpenSSL-VC-32-STATIC no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl => "C:\\Perl64\\bin\\perl.exe",
  perlargv => [ "VC-WIN32", "no-shared", "--prefix=C:\\Build-OpenSSL-VC-32-STATIC" ],
  prefix => "C:\\Build-OpenSSL-VC-32-STATIC",
  processor => "",
