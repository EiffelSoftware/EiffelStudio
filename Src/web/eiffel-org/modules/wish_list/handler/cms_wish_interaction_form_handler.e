...
class
	CMS_WISH_INTERACTION_FORM_HANDLER
...
feature {NONE} --GET Implementation
	...
	edit_wish_interaction_form (req: WSF_REQUEST; res: WSF_RESPONSE; a_wish_id:INTEGER; a_interaction_id: INTEGER)
		...
		local
			r: CMS_RESPONSE
			l_form: CMS_WISH_INTERACTION_FORM_VIEW
		do
			if attached current_user_name (req) as l_user then
				create l_form.make (wish_api.status, wish_api.categories)
				l_form.set_wish (wish_api.wish_by_id (a_wish_id))
				create {GENERIC_VIEW_CMS_RESPONSE} r.make (req, res, api)
				if attached template_block ("wish_interaction_form", r) as l_tpl_block then
					...
					if l_form.id > 0 then
						...
						if attached wish_api.wish_interactions_attachments (a_wish_id, l_form.id ) as l_attachments then
...
feature {NONE} -- POST Implementation
	...
	save_wish_interaction (req: WSF_REQUEST; res: WSF_RESPONSE)
		...
		local
			l_form: CMS_WISH_INTERACTION_FORM_VIEW
			r: CMS_RESPONSE
			l_interaction: CMS_WISH_LIST_INTERACTION
		do
			if attached current_user (req) as l_user then
				if
					attached {WSF_STRING} req.path_parameter ("wish_id") as l_wish_id and then l_wish_id.is_integer and then
					attached  {CMS_WISH_LIST} wish_api.wish_by_id (l_wish_id.integer_value) as l_wish
				then
					...
					if l_form.is_valid_form then
						...
						wish_api.save_wish_interaction (l_interaction)
						l_form.set_id (l_interaction.id)

							-- Update wish item.
						if l_form.selected_status > 0 then
							l_wish.set_status (create {CMS_WISH_LIST_STATUS}.make (l_form.selected_status, ""))
						end
						...
						if attached template_block ("wish_interaction_form", r) as l_tpl_block then
							...
							if attached wish_api.wish_interactions_attachments (l_wish.id, l_form.id ) as l_attachments then
								l_tpl_block.set_value (l_attachments, "uploaded_files")
							end
...
feature {NONE} -- Upload and temporary file processing.
	...
	upload_temporary_files_html (req: WSF_REQUEST; a_form: CMS_WISH_INTERACTION_FORM_VIEW)
				...
				elseif attached {WSF_STRING} req.form_parameter ("uploaded_files") as l_file and then
					...
					across wish_api.wish_attachments (a_form.id) as c loop
						if not c.item.name.same_string (l_file.value) then
							wish_api.remove_wish_interaction_attachment_by_name (l_wish.id, a_form.id, c.item.name)
						end
					end
				...
				if attached a_form.temporary_files as l_files then
					across l_files as c loop
						...
						if c.item.size.to_natural_64 <= ({NATURAL_64}10*1024*1024) then
							wish_api.upload_wish_interaction_attachment (l_wish.id, a_form.id, c.item)
						else
...
feature {NONE} -- Form Processing
	...
	extract_form_data (req: WSF_REQUEST): CMS_WISH_INTERACTION_FORM_VIEW
		...
		do
			create Result.make (wish_api.status, wish_api.categories)
