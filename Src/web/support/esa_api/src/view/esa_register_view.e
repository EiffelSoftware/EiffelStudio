...
feature -- Status Report
	...
	is_valid_form: BOOLEAN
		...
		do
			...
			if attached password as l_password and then
			   l_password.is_empty then
			   	l_errors.put ("Password is required", "password")
			end
			if attached check_password as l_check_password and then
				  l_check_password.is_empty then
				l_errors.put ("Check Password is required", "check_password")
			end
			if attached user_name as l_user_name and then
				  l_user_name.is_empty then
				l_errors.put ("Username is required", "user_name")
			end
			if attached email as l_email and then
				 l_email.is_empty then
				l_errors.put ("Email is required", "email")
			end
			if attached answer as l_answer and then
				l_answer.is_empty then
				l_errors.put ("Answer is required", "answer")
			end
			if attached first_name as l_first_name and then
				l_first_name.is_empty then
				l_errors.put ("First name is required", "first_name")
			end
			if attached last_name as l_last_name and then
				l_last_name.is_empty then
				l_errors.put ("Last name is required", "last_name")
			end

			if attached password as l_password and then
			   attached check_password as l_check_password then
