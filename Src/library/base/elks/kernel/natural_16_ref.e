...
feature -- Conversion
	...
	to_binary_string: STRING
		...
		do
			...
			result_valid_count: Result.count = {PLATFORM}.natural_16_bits
	...
	to_octal_string: STRING
		...
		do
			from
				i := ({PLATFORM}.natural_16_bits / 3).ceiling_real_64.truncated_to_integer
				create Result.make_filled ('0', i)
				val := item
			until
				i = 0
			...
			result_not_void: Result /= Void
			result_valid_count: Result.count = ({PLATFORM}.natural_16_bits / 3).ceiling_real_64.truncated_to_integer
	...
	to_hex_string: STRING
		...
		do
			...
			result_not_void: Result /= Void
			result_valid_count: Result.count = {PLATFORM}.natural_16_bits // 4
...
feature -- Bit operations
	...
	frozen set_bit (b: BOOLEAN; n: INTEGER): NATURAL_16
		...
		do
			if b then
				Result := item | ({NATURAL_16} 1 |<< n)
			else
				Result := item & ({NATURAL_16} 1 |<< n).bit_not
			end
		end
