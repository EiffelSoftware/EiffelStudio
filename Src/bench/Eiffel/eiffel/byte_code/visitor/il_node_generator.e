...
feature {NONE} -- Visitors
	...
	process_assign_b (a_node: ASSIGN_B) is
		...
		do
			generate_il_line_info (a_node, True)
			process_pragma (a_node.line_pragma)
...
feature {NONE} -- Implementation
	...
	process_pragma (a_pragma: BREAK_AS) is
			-- Process pragma `a_pragma'.
		local
			l_parser: LINE_PRAGMA_PARSER
		do
			if (a_pragma /= Void) and then (system.line_generation or context.workbench_mode) then
				create l_parser
				l_parser.parse (a_pragma.literal_text (Void))
				if l_parser.is_pragma then
					...
					elseif l_parser.is_hidden then
						...
						il_generator.set_pragma_offset (l_parser.line_number - a_pragma.start_location.line)
