...
invariant
	...
	basic_type: is_basic_type implies (not is_array_basic_type and not is_interface and not is_interface_pointer
							and not is_structure and not is_structure_pointer)
	...
	structure: is_structure implies (not is_basic_type and not is_array_basic_type and 
							not is_interface and not is_interface_pointer and not is_structure_pointer)

	structure_pointer: is_structure_pointer implies (not is_basic_type and not is_array_basic_type and 
							not is_interface and not is_interface_pointer and not is_structure)

	interface: is_interface implies (not is_basic_type and not is_array_basic_type and 
							not is_structure and not is_structure_pointer and not is_interface_pointer)

	interface_pointer: is_interface_pointer implies (not is_basic_type and not is_array_basic_type and 
							not is_structure and not is_structure_pointer and not is_interface)
	...
	need_generate_ce: need_generate_ce implies (ce_function_body /= Void and then not ce_function_body.empty)
						and (ce_function_return_type /= Void and then not ce_function_return_type.empty)
						and (ce_function_signature /= Void and then not ce_function_signature.empty)

	need_generate_ec: need_generate_ec implies (ec_function_body /= Void and then not ec_function_body.empty)
						and (ec_function_return_type /= Void and then not ec_function_return_type.empty)
						and (ec_function_signature /= Void and then not ec_function_signature.empty)
