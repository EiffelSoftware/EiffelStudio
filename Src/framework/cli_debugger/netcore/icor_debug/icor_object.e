...
feature {ICOR_EXPORTER} -- Initialisation
	...
	make_by_pointer (a_item: POINTER)
		...
		do
			debug ("ICOR_OBJECT")
				io.put_string ("[" + generating_type.name + "].make_by_pointer(" + a_item.out + ") called")
				io.put_new_line
			end
			item := a_item
			init_icor
...
feature {ICOR_OBJECTS_MANAGER_I} -- Special feature for ICOR_OBJECTS_MANAGER
