...
void ecom_control_library::IEnumOleUndoUnits_impl_proxy::ccom_remote_next(  /* [in] */ EIF_INTEGER celt,  /* [out] */ EIF_OBJECT rgelt,  /* [out] */ EIF_OBJECT pcelt_fetched )
...
-----------------------------------------------------------*/
{
	...
	ULONG tmp_celt = 0;
	tmp_celt = (ULONG)celt;
	ecom_control_library::IOleUndoUnit * * tmp_rgelt = 0;
	tmp_rgelt = (ecom_control_library::IOleUndoUnit * *)grt_ec_control_interfaces2.ccom_ec_pointed_cell_160 (eif_access (rgelt), NULL);
	ULONG * tmp_pcelt_fetched = 0;
	tmp_pcelt_fetched = (ULONG *)rt_ec.ccom_ec_pointed_unsigned_long (eif_access (pcelt_fetched), NULL);
	
	hr = p_IEnumOleUndoUnits->RemoteNext(tmp_celt,tmp_rgelt,tmp_pcelt_fetched);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
	...
	grt_ce_control_interfaces2.ccom_ce_pointed_cell_160 ((ecom_control_library::IOleUndoUnit * *)tmp_rgelt, rgelt);
