indexing
	description: "Application storer."
	id: "$Id$"
	date: "$Date$"
	revision: "$Revision$"
...
feature {NONE} -- Private attributres
	...
	tmp_store (dir_name: STRING) is
		...
		do
			...
			loop
				...
				create s_circle.make (app_circle)
				stored_circles.extend (s_circle)
				app_figures.forth
			end

			create stored_lines.make
			from
			...
			until
				app_lines.after
			loop
				create s_line.make (app_lines.line)
	...
	rebuild_graph is
		...
		do
			...
			loop
				...
				create new_table.make (subtab.count)
...
feature {NONE} -- Implementation
	...
	build_stored_graph is
		local
			...
			g: GRAPH_ELEMENT
		do
			create stored_graph.make (Shared_app_graph.count)
			from
