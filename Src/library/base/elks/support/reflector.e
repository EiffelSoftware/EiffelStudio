...
feature -- Creation
	...
	new_special_any_instance (type_id, a_capacity: INTEGER): SPECIAL [detachable ANY]
		...
		do
			Result := {ISE_RUNTIME}.new_special_any_instance_of (type_id, a_capacity)
		ensure
			instance_free: class
...
feature -- Status report
	...
	is_special_any_type (type_id: INTEGER): BOOLEAN
		...
		require
			type_id_nonnegative: type_id >= 0
		do
			Result := {ISE_RUNTIME}.is_special_any_type (type_id)
		ensure
			instance_free: class
		end
	...
	is_special_type (type_id: INTEGER): BOOLEAN
		...
		require
			type_id_nonnegative: type_id >= 0
		do
			Result := {ISE_RUNTIME}.is_special_type (type_id)
		ensure
			instance_free: class
		end
