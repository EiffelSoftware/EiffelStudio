...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_220 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_ISE_c.ccom_ec_pointed_cell_220 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
