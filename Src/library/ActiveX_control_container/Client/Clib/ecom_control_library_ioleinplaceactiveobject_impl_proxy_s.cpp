...
void ecom_control_library::IOleInPlaceActiveObject_impl_proxy::ccom_remote_translate_accelerator()
...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleInPlaceActiveObject->RemoteTranslateAccelerator ();
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IOleInPlaceActiveObject_impl_proxy::ccom_remote_resize_border(  /* [in] */ ecom_control_library::tagRECT * prc_border,  /* [in] */ GUID * riid,  /* [in] */ ecom_control_library::IOleInPlaceUIWindow * p_uiwindow,  /* [in] */ EIF_INTEGER f_frame_window )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleInPlaceActiveObject->RemoteResizeBorder(prc_border,riid,p_uiwindow,tmp_f_frame_window);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
