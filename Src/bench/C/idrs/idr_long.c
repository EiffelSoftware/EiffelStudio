...
long *lp;
{
	...
	if (idrs->i_op == IDR_ENCODE) {
		if (size == 4) {						/*encode for long = 4 bytes */
			value = htonl((uint32)(*lp));
			bcopy(&value, idrs->i_ptr, size);
			idrs->i_ptr += size;
		}
		else {									/*encode for long = 8bytes */
			uint32 upper, lower;
			long temp;
		...
		if (size == 4) {						/* decode a 4 byte long */
			...
			*lp = (long) ntohl(value);
			...
			idrs->i_ptr += 4;
			if ((size = sizeof(long)) == 4) {			/*if the data has come from a 8 byte */
			...
			else {								/* rejoin the upper and lower parts */ 
