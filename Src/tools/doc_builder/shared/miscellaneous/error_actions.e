...
feature -- Access
	...
	highlight_error (a_item: EV_LIST_ITEM) is
		...
		do
			...
			a_item.pointer_double_press_actions.force_extend (agent curr_doc.highlight_error (l_no, l_pos))
		end
	...
	load_file (a_item: EV_LIST_ITEM) is
		...
		require
			item_not_void: a_item /= Void
		do
			a_item.pointer_double_press_actions.force_extend (agent Shared_document_editor.load_document_from_file (a_item.text))
		end
