...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_122 (rgelt, NULL));
		if (*rgelt != NULL)
			(*rgelt)->AddRef ();
	}
	...
	if (*rgelt != NULL)
		(*rgelt)->Release ();
	grt_ec_ISE_c.ccom_ec_pointed_cell_122 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_95 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_ISE_c.ccom_ec_pointed_cell_95 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
...
STDMETHODIMP ecom_eiffel_compiler::IEnumParameter_impl_stub::GetTypeInfo( unsigned int itinfo, LCID lcid, ITypeInfo **pptinfo )
...
STDMETHODIMP ecom_eiffel_compiler::IEnumParameter_impl_stub::GetTypeInfoCount( unsigned int * pctinfo )
...
STDMETHODIMP ecom_eiffel_compiler::IEnumParameter_impl_stub::GetIDsOfNames( REFIID riid, OLECHAR ** rgszNames, unsigned int cNames, LCID lcid, DISPID *rgdispid )
...
STDMETHODIMP ecom_eiffel_compiler::IEnumParameter_impl_stub::Invoke( DISPID dispID, REFIID riid, LCID lcid, unsigned short wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, unsigned int *puArgErr )
