...
void ecom_control_library::IPropertyBag_impl_proxy::ccom_remote_read(  /* [in] */ EIF_OBJECT psz_prop_name,  /* [out] */ VARIANT * p_var,  /* [in] */ ecom_control_library::IErrorLog * p_error_log,  /* [in] */ EIF_INTEGER var_type,  /* [in] */ IUnknown * p_unk_obj )
...
-----------------------------------------------------------*/
{
	...
	ULONG tmp_var_type = 0;
	tmp_var_type = (ULONG)var_type;
	
	hr = p_IPropertyBag->RemoteRead(tmp_psz_prop_name,p_var,p_error_log,tmp_var_type,p_unk_obj);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
