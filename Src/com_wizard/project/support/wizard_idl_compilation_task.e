	...
	internal_execute is
		...
		do
			...
			if environment.is_new_component and environment.marshaller_generated and not environment.abort then
				message_output.add_title (Current, "Compiling marshaller DLL GUIDs")
				compiler.compile_iid
				if not environment.abort then
					progress_report.step
					message_output.add_title (Current, "Compiling marshaller DLL data")
					compiler.compile_data
				end
				if not environment.abort then
					progress_report.step
					message_output.add_title (Current, "Compiling marshaller DLL interface")
					compiler.compile_ps
				end
				if not environment.abort then
					progress_report.step
					message_output.add_title (Current, "Linking marshaller DLL")
					compiler.link
				end
				if not environment.abort then
					progress_report.step
					message_output.add_title (Current, "Registering marshaller DLL")
