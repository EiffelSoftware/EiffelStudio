note
	description: "MySQL DATETIME Value"
	author: "haroth@student.ethz.ch"
	date: "$Date$"
	revision: "$Revision$"

class
	MYSQLI_DATETIME_VALUE
...
feature{NONE} -- Initialization
	...
	make_as_string (a_value: STRING)
		...
		do
			make_as_date_time (default_datetime_format_code_string.create_date_time (a_value))
		end
...
feature -- Access (Field Type)
	...
	is_datetime_value: BOOLEAN = True
...
feature -- Access (Set Type)
	...
	as_set: SET [STRING]
		...
		do
			...
			Result := l_linked_set
			raise ("MYSQLI_DATETIME_VALUE.as_set: current 'value' is not representable as a set.")
		end
