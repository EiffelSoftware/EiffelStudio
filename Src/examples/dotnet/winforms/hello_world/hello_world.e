	...
	initialize_components is
		...
		do
			...
			create my_text_box.make
				...
				-- Initialize window.
			my_window.set_text (("Accessibility").to_cil)
--			my_window.set_auto_scale_base_size (create {DRAWING_SIZE}.make_from_width_and_height (5, 13))
			...
			my_window.set_accessible_role (feature {WINFORMS_ACCESSIBLE_ROLE}.window)--System.Windows.Forms.AccessibleRole.Window)
...
--			my_text_box.set_size (create {DRAWING_SIZE}.make_from_width_and_height (360, 20))
			...
			my_window.get_controls.add (my_button)
			my_window.get_controls.add (my_text_box)
		end
...
feature {NONE} -- Implementation
	...
	my_button_clicked (sender: SYSTEM_OBJECT; args: EVENT_ARGS) is
		...
		local
			...
			dummy: WINFORMS_DIALOG_RESULT
		do
			msg := msg.concat_string_string_string (("Text is : '").to_cil, my_text_box.get_text, ("'").to_cil)
