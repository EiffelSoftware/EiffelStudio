...
feature -- Status report
	...
	is_relation_type_valid (a_type: like relation_type): BOOLEAN is
		...
		do
			Result :=
				a_type = class_client_relation or
				a_type = class_indirect_client_relation
		ensure then
			good_result:
				Result implies (
					a_type = class_client_relation or
...
feature{NONE} -- Implementation
	...
	find_client_classes (a_class_c: CLASS_C) is
			-- Find all clients of `a_class_c' and put them in `candidate_class_list' and `candidate_class_table'.
		local
			l_list: LIST [CLASS_C]
	...
	find_indirect_client_classes (a_class_c: CLASS_C) is
			-- Find indirect clients of `a_class_c' and put them in `candidate_class_list' and `candidate_class_table'.
		local
		...
		do
			...
			if not l_clients.is_empty then
				...
				until
					l_clients.after
				loop
					find_client_classes (l_clients.item)
					l_clients.forth
				end
			end
		end
...
feature{NONE} -- Implementation
	...
	finder_agent_table: HASH_TABLE [PROCEDURE [ANY, TUPLE [CLASS_C]], QL_CLASS_CLIENT_RELATION] is
			-- Table for finders of certain kind of class clients (client, indirect client) relation.
		...
		do
			if internal_finder_agent_table = Void then
				...
				internal_finder_agent_table.put (agent find_client_classes, class_client_relation)
				internal_finder_agent_table.put (agent find_indirect_client_classes, class_indirect_client_relation)
			end
