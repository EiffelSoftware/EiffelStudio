...
feature -- Initialization
	...
	make (min_index, max_index: INTEGER) is
		...
		require
			valid_bounds: (agent (l_min_index: INTEGER): INTEGER do $MELT1; Result := l_min_index end).item([min_index]) <= 
						  (agent (l_max_index: INTEGER): INTEGER do $MELT2; Result := l_max_index + 1 end).item([max_index])
		do
			lower := min_index
			upper := max_index
			...
			lower_set: (agent (l_min_index: INTEGER): BOOLEAN do $MELT4; Result := (agent lower).item ([]) = l_min_index end).item ([min_index])
			upper_set: (agent (l_max_index: INTEGER): BOOLEAN do $MELT5; Result := (agent upper).item ([]) = l_max_index end).item ([max_index])
