note
	description: "Summary description for {CMS_WISH_LIST_HANDLER}."
	date: "$Date$"
	revision: "$Revision$"

class
	CMS_WISH_LIST_HANDLER
...
feature -- GET
	...
	wish_list (req: WSF_REQUEST; res: WSF_RESPONSE)
			-- List of wish liar.
		local
			l_input_validator: CMS_WISH_LIST_INPUT_VALIDATOR
			l_categories: LIST [CMS_WISH_LIST_CATEGORY]
			l_status: LIST [CMS_WISH_LIST_STATUS]
			l_pages: INTEGER
			l_row: LIST [CMS_WISH_LIST]
			l_view: CMS_WISH_LIST_VIEW
			r: CMS_RESPONSE
		...
		do
			...
			if not l_input_validator.has_error then
				l_categories := wish_api.categories
				l_status := wish_api.status

				l_pages := wish_api.row_count_wish_list (l_input_validator.category, l_input_validator.status_selected, l_input_validator.filter, l_input_validator.filter_content)
				...
				l_row := wish_api.wish_list (l_input_validator.page, l_input_validator.size, l_input_validator.category, l_input_validator.status_selected, l_input_validator.orderby, l_input_validator.dir_selected, l_input_validator.filter, l_input_validator.filter_content )
				...
				if attached template_block ("wish_list", r) as l_tpl_block then
					create {CMS_WISH_LIST_STATUS_INSPECTOR} tpl_inspector.register (({detachable CMS_WISH_LIST_STATUS}).out)
					create {CMS_WISH_LIST_CATEGORY_INSPECTOR} tpl_inspector.register (({detachable CMS_WISH_LIST_CATEGORY}).out)
...
feature -- Helpers
	...
	retrieve_status_query (a_status: LIST [CMS_WISH_LIST_STATUS]): STRING_8
		do
			Result := "0&"
			across
				a_status as c
	...
	set_selected_status (a_status: LIST [CMS_WISH_LIST_STATUS]; a_selected_status:  INTEGER)
	...
	mark_selected_status (a_status: LIST [CMS_WISH_LIST_STATUS]; a_status_selected: LIST[ INTEGER] )
			-- Set the current selected status
		do
	...
	set_selected_category (a_categories: LIST [CMS_WISH_LIST_CATEGORY]; a_selected_category:  INTEGER)
