...
#include "ecom_eiffel_compiler_IEnumCompletionEntry_impl_stub_s.h"
...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_ISE.ccom_ce_pointed_cell_239 (rgelt, NULL));
		if (*rgelt != NULL)
			(*rgelt)->AddRef ();
	}
	...
	if (*rgelt != NULL)
		(*rgelt)->Release ();
	grt_ec_ISE.ccom_ec_pointed_cell_239 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_ISE.ccom_ce_pointed_cell_236 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_ISE.ccom_ec_pointed_cell_236 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
