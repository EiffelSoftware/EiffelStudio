...
feature -- Element update
	...
	add_external
		...
		do
			...
			l_external := configuration_window.conf_factory.new_external_make ("new", target)
...
feature {NONE} -- Section properties
	...
	add_text: STRING_GENERAL
		...
		do
			Result := conf_interface_names.external_add_make
		end

	add_icon: EV_PIXMAP
			-- Menu entry icon to add a new item.
		do
			Result := conf_pixmaps.new_makefile_icon
		end

	add_button: SD_TOOL_BAR_BUTTON
			-- Toolbar button to add a new item.
		do
			Result := toolbar.add_make_button
		end
