...
feature -- C code generation
	...
	analyze is
		...
		do
			...
			expression.analyze
			create result_register.make (boolean_c_type)
		...
		do
			...
			if target_type.is_expanded and source_type.is_none then
				...
				if not (register_propagated and expression.is_simple_expr)
					and then not register_for_metamorphosis
				then
					register.print_register
					buf.put_string (" = ")
						...
						then
							...
							if target_type.is_basic then
								...
								if context.workbench_mode then
									target_class_type.associated_class_type.skeleton.generate_workbench_size (buf)
								else
									target_class_type.associated_class_type.skeleton.generate_size (buf)
								end
								buf.put_string (gc_rparan_semi_c)
								buf.put_new_line
							end
