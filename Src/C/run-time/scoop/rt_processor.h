...
doc:		<field name="current_impersonated_call" type="struct call_data*"> The call that this processor is currently impersonating. This will be traced during GC marking.
doc:			NOTE: In fact this should be a stack, because a processor can impersonate multiple calls in its C stack (due to separate callbacks). However, we only need to protect the call data until
doc:			the stack frame of the Eiffel call is fully initialized, so we can use a simple field instead and live with the fact that we sometimes overwrite the information of another impersonated call. </field>
...
struct rt_processor {
	...
	struct call_data* current_impersonated_call;
	EIF_SCP_PID pid;
	EIF_SCP_PID client;
