	...
	make (a_name: STRING) is
		...
		do
			...
			if string_name.count > 0 then
				create directory.make (string_name)
			end
		...
		do
			if not rescued then
				if string_name.count > 0 then
					...
					create directory_stream.make (directory)
	...
	read_entry is
		...
		do
			...
			elseif old_end_of_input then
				...
				directory_stream.read
				last_entry := directory_stream.item
...
feature {NONE} -- Implementation
	...
	old_end_of_input: BOOLEAN is
		...
		do
			...
			Result := directory_stream.end_of_stream
...
invariant
	...
	directory_not_void: string_name.count > 0 implies directory /= Void
