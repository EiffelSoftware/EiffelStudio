...
deferred class
	PDB_MD_TABLE [G -> PDB_MD_TABLE_ENTRY]
...
feature {NONE} -- Initialization
	...
	make (a_tables: PDB_MD_TABLES; pe: PDB_FILE; tb_id: like table_id; nb: NATURAL_32)
		do
			tables := a_tables
			table_id := tb_id
...
feature -- Read
	...
	read (pe: PDB_FILE)
		...
		do
			...
			debug ("pe_analyze")
				io.error.put_string ("Read table " + {PDB_MD_TABLES}.table_name (table_id) + "  " + table_id.to_hex_string + " (" + count.out + ") at 0x"+ address.to_hex_string +"%N")
			end
