indexing
	description: "Objects that represent EV_TREE_ITEM_CHECK_ACTION_SEQUENCE"
...
class
	GB_EV_TREE_ITEM_CHECK_ACTION_SEQUENCE
...
feature -- Access
	...
	argument_names: ARRAYED_LIST [STRING] is
		...
		once
			create Result.make (0)
			Result.extend ("a_tree_item")
		end
		
	display_agent (name: STRING; string_handler: ORDERED_STRING_HANDLER): PROCEDURE [ANY, TUPLE [EV_TREE_ITEM]] is
...
feature {NONE} -- Implementation
	...
	internal_display_agent (a_tree_item: EV_TREE_ITEM; name: STRING; string_handler: ORDERED_STRING_HANDLER) is
			-- Display all other arguments of `Current' on `string_handler', prepended
			-- with `name' fired.
		do
			string_handler.record_string (name + " fired.%Nitem : " + a_tree_item.parent.index_of (a_tree_item, 1).out)
		end
...
end -- class GB_EV_TREE_ITEM_CHECK_ACTION_SEQUENCE
