	...
	finalize
		...
		do
			...
			if operation = decrypt_mode and then attached {SSL_MODE_WITH_AUTHENTICATION_TAG} mode as l_mode and then tag = Void then
				...
				if l_res = 0 then
					if {SSL_ERROR_FACTORY}.retrieve_errors = Void and then attached {SSL_GCM_MODE} mode as l_mode then
						raise_exception ("Invalid Tag")
					else
						if attached {LIST [SSL_ERROR]} ({SSL_ERROR_FACTORY}.retrieve_errors) as l_errors and then (l_errors.at (1).lib_and_reason_match ({SSL_ERROR_EXTERNALS}.ERR_LIB_EVP, {SSL_ERROR_EXTERNALS}.EVP_R_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH)) then
