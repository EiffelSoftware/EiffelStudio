...
feature -- File names
	...
	system_general: FILE_NAME is
		...
		do
			if environment.preference_file_name	/= Void
			then
				!! Result.make_from_string	(environment.preference_file_name)
			else
				!! Result.make_from_string	( environment.project_directory	)
				Result.extend	("system.ecr")
			end
