...
public void epop();					/* Pops values off a stack */
private int extend();				/* Stack extension w/ urgent chunks */
...
register2 int nb_items;				/* Number of items to be popped */
{
	 ...
	 * more items than there actually were held in the stack. So panic.
	 * Otherwise 'top' is correctly set and 's' is the new current chunk.
	 */
	if (s == (struct stchunk *) 0)
		panic("run-time stack botched");
#endif
...
register1 int num;
{
	 ...
	 * is likely). In that case however, we do not panic, we raise an "Out of
	 * memory" exception.
	 */
...
register1 char **top;
{
	...
	if (loc_set.st_cur == (struct stchunk *) 0)	/* No previous chunk ? */
		panic("local stack underflow");			/* That's a critical event */
#endif
	...
	if (top < loc_set.st_cur->sk_arena || top > loc_set.st_end)
		panic("local stack inconsistency");
#endif
...
private void collect_stats();	/* Gives statistics on collector's stack */
private void stack_stats();		/* Gives statistics on local stack */
