indexing
	description: "Comparator to compare positions of two EV_GRID_ITEM objects by columns"
...
class
	EVS_GRID_ITEM_COLUMN_POSITION_COMPARATOR
...
feature -- Comparator
	...
	comparator (a_item, b_item: EVS_GRID_COORDINATED): BOOLEAN is
			...
			-- It will compare column first, and then the row.
		local
			l_ax, l_ay: INTEGER
			l_bx, l_by: INTEGER
		do
			...
			if l_ax < l_bx then
				Result := True
			elseif l_ax = l_bx and then l_ay < l_by then
				Result := True
			end
		end
