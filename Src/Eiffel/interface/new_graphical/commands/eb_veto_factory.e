...
feature -- Various thing you can allow.
	...
	can_drop_debuggable_feature_or_class (a_stone: ANY): BOOLEAN
		...
		do
			if attached {STONE} a_stone as l_stone and then l_stone.is_valid then
				if attached {FEATURE_STONE} l_stone as l_fst then
					Result := attached l_fst.e_feature as f and then f.is_debuggable
				elseif attached {CLASSC_STONE} l_stone as l_cst then
