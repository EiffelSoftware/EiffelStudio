note
	description: "MySQL MEDIUMINT Value"
	author: "haroth@student.ethz.ch"
	date: "$Date$"
	revision: "$Revision$"

class
	MYSQLI_MEDIUMINT_VALUE
...
feature{NONE} -- Initialization
	...
	make (a_value: INTEGER_32)
		...
		do
			if a_value.bit_and (8388608) > 0 then
				value := - 1 - a_value.bit_xor (8388607).bit_and (8388607).to_integer_32
			else
				value := a_value.bit_and (8388607)
			end
		end

	make_as_integer_32 (a_value: INTEGER_32)
			-- Set `value' to `a_value'.
		do
			make (a_value)
		end

	make_as_natural_32 (a_value: NATURAL_32)
			-- Set `value' to `a_value'.
		do
			make (a_value.to_integer_32)
			is_unsigned := True
		end
...
feature{MYSQLI_EXTERNALS} -- Internals
	...
	bind (a_bind: POINTER; a_buffer, a_is_null, a_length: MANAGED_POINTER)
		...
		do
			...
			a_buffer.put_natural_32 (value.to_natural_32, 0)
			c_struct_mysql_bind_set_buffer (a_bind, a_buffer.item)
			if is_unsigned then
				c_struct_mysql_bind_set_is_unsigned (a_bind, 1)
			end
		end
...
feature -- Access (Field Type)
	...
	is_mediumint_value: BOOLEAN = True
...
feature -- Access (Types)
	...
	as_natural_32: NATURAL_32
		...
		do
			Result := value.to_natural_32.bit_and (16777215)
		end

	as_natural_64: NATURAL_64
			-- This value as a NATURAL_64
		do
			Result := value.to_natural_64.bit_and (16777215)
		end
...
feature -- Access (Date and Time Types)
	...
	as_date_and_time_tuple: TUPLE [years, months, days, hours, minutes, seconds: INTEGER_32]
		...
		do
			...
			raise ("MYSQLI_MEDIUMINT_VALUE.as_date_and_time_tuple: current 'value' is not representable as a date and time tuple.")
		end
	...
	as_timestamp: DATE_TIME
		...
		do
			...
			raise ("MYSQLI_MEDIUMINT_VALUE.as_timestamp: current 'value' is not representable as a timestamp.")
		end
	...
	as_date: DATE
		...
		do
			create Result.make_now
			raise ("MYSQLI_MEDIUMINT_VALUE.as_date: current 'value' is not representable as a date.")
		end
	...
	as_time: TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_MEDIUMINT_VALUE.as_time: current 'value' is not representable as a time.")
		end
	...
	as_datetime: DATE_TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_MEDIUMINT_VALUE.as_datetime: current 'value' is not representable as a datetime.")
		end

	as_year: INTEGER
			-- This value as a INTEGER
		do
			raise ("MYSQLI_MEDIUMINT_VALUE.as_year: current 'value' is not representable as a year.")
		end
...
feature -- Access (Set Type)
	...
	as_set: SET [STRING]
		...
		do
			...
			Result := l_linked_set
			raise ("MYSQLI_MEDIUMINT_VALUE.as_set: current 'value' is not representable as a set.")
		end
