	...
	read_header (a_count: NATURAL_32)
		...
		do
			...
			loop
				...
				l_type_string := l_deser.read_string_8
				l_new_dtype := l_int.dynamic_type_from_string (l_type_string)
				if l_new_dtype = -1 then
					set_error (error_factory.new_missing_type_error (l_type_string))
