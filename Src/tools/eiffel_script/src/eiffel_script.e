...
--				if op = op_build then
--					if target = Void then
--						if v.ends_with_general (".ecf") then
--							create target.make_from_string (v)
--						else
--							report_error ({STRING_32} "Ignore argument: " + v)
--						end
--					elseif l_build_output = Void then
--						create l_build_output.make_from_string (v)
--						l_build_output := l_build_output.absolute_path.canonical_path
--					else
--						report_error ({STRING_32} "Ignore argument: " + v)
--					end
--				elseif op = op_launch and target /= Void then
--						-- args for execution
--					args.force (v)
--				else
--					if v.same_string ("-v") or v.same_string ("--verbose") then
--						is_verbose := True
--					elseif v.same_string ("-b") or v.same_string ("--build") then
--						is_build_forced := True
--					elseif v.same_string_general ("-h") or v.same_string_general ("--help") then
--						op := op_help
--					elseif v.ends_with_general (".ecf") then
--						op := op_launch
--						create target.make_from_string (v)
--					elseif v.same_string ("build") then
--						op := op_build
--					else
--							-- Ignore args
--						report_error ({STRING_32} "Ignore argument: " + v)
...
--				display_usage
--			else
--				target := target.absolute_path.canonical_path
--				inspect op
--				when op_launch then
--					launch (target, args)
--				when op_build then
--					build (target, l_build_output)
--				else
--					display_usage
--				end
--			end
