...
class
	ECD_PROPERTY_REFERENCE_EXPRESSION
...
feature -- Access
	...
	code: STRING is
			...
			-- | OR		:= "feature {`target_object'}.set/get_`property_name'" if typeof `target_object' is ECD_TYPE_REFERENCE_EXPRESSION
			-- Eiffel code of property reference expression
		local
			target_name: STRING
			type_ref_exp: ECD_TYPE_REFERENCE_EXPRESSION
		do
			check
...
end -- class ECD_PROPERTY_REFERENCE_EXPRESSION
