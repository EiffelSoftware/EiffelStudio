	...
	initialize is
		...
		do
			...
			if feature {EV_GTK_EXTERNALS}.gtk_is_window (c_object) then
				real_signal_connect (c_object, "configure-event", agent (App_implementation.gtk_marshal).on_size_allocate_intermediate (object_id, ?, ?, ?, ?), size_allocate_translate_agent)
			else
				real_signal_connect (c_object, "size-allocate", agent (App_implementation.gtk_marshal).on_size_allocate_intermediate (object_id, ?, ?, ?, ?), size_allocate_translate_agent)
			end
