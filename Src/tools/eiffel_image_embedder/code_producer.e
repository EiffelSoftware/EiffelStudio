...
feature -- Commands
		...
		do
			...
			loop
				if l_iterator.after then
					...
					Result.append (tab (0) + "c_colors_" + l_feature_count.out + " (a_ptr: POINTER; a_offset: INTEGER) is" + new_line)
	...
	build_colors_code: STRING
		...
		do
			...
			Result.append (tab (0) + "build_colors (a_ptr: POINTER) is" + new_line)
	...
	build_fill_memory_code (a_colors: ARRAYED_LIST [ARRAYED_LIST [NATURAL_32]]; a_width, a_height: INTEGER): STRING
		...
		do
			...
			Result.append (new_line)
			Result.append (tab (1) + "fill_memory is" + new_line)
