...
class EXECUTION_ENVIRONMENT_32
...
feature -- Access
	...
	default_shell: STRING_32
			-- Default shell
		once
			if attached get ("SHELL") as r then
				Result := r
			else
	...
	get (s: STRING_32): detachable STRING_32
...
feature -- Status setting
	...
	change_working_directory (path: STRING_32)
			-- Set the current directory to `path'
		do
			{ENVIRONMENT}.set_current_directory (path)
		end

	put (value, key: STRING_32)
...
feature {NONE} -- Implementation
	...
	internal_launch_from_local_volume (s: STRING_32; should_wait: BOOLEAN)
			-- Pass to the operating system an asynchronous request to
			-- execute `s'.
			-- If `s' is empty, use the default shell as command.
			-- If should_wait is true, then wait for the child to exit
