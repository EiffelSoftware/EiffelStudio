...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_Eif_compiler.ccom_ce_pointed_cell_168 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_Eif_compiler.ccom_ec_pointed_cell_168 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_Eif_compiler.ccom_ce_pointed_cell_175 (rgelt, NULL));
	}
	...
	if (*rgelt != NULL)
		rt_ce.free_memory_bstr (*rgelt);
	grt_ec_Eif_compiler.ccom_ec_pointed_cell_175 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
