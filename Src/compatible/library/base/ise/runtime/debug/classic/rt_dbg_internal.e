...
feature {NONE} -- Factory
	...
	frozen object_record (i: INTEGER; obj: ANY): detachable RT_DBG_VALUE_RECORD
		...
		do
			ft := field_type (i, obj)
			Result := object_attribute_record (field_offset (i, obj), c_rt_field_type (i, ft), obj)
			inspect ft
			when Integer_8_type then
			...
			when Reference_type then
				create {RT_DBG_FIELD_RECORD [detachable ANY]} Result.make (obj, i, ft, reference_field (i, obj))
