...
feature -- Formatting
	...
	format_class (a_node: CLASS_AS) is
			-- Format `a_node'.
		require
			a_node_not_void: a_node /= Void
		do
			context.clear
			match_list := a_node.match_list
			last_index := 0
...
feature{NONE} -- Implementation
	...
	process_before_leaves (ind: INTEGER) is
			-- Process all left leaves in `match_list' before index `ind'.
		require
			valid_index: ind > 0 and then ind <= match_list.count
	...
	put_string (l_as: LEAF_AS) is
			-- Process `l_as'.
		require
			l_as_in_list: l_as.index >0 and then l_as.index <= match_list.count
		do
