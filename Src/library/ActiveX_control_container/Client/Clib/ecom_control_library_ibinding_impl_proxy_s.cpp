...
void ecom_control_library::IBinding_impl_proxy::ccom_get_bind_result(  /* [out] */ GUID * pclsid_protocol,  /* [out] */ EIF_OBJECT pdw_result,  /* [out] */ EIF_OBJECT psz_result,  /* [in] */ EIF_INTEGER dw_reserved )
...
-----------------------------------------------------------*/
{
  ...
  hr = p_IBinding->GetBindResult(pclsid_protocol,tmp_pdw_result,tmp_psz_result,&tmp_dw_reserved);
  if (FAILED (hr))
  {
    if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
      com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
