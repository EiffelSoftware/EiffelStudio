	...
	context_menu_handler (a_menu: EV_MENU; a_target_list: ARRAYED_LIST [EV_PND_TARGET_DATA]; a_source: EV_PICK_AND_DROPABLE; a_pebble: ANY)
		...
		do
			...
			if l_pebble_type.is_equal ({CLASSC_STONE}) then
					...
					-- Alter first item text to match context.
				l_text := interface_names.m_pick.as_string_32 + " " + interface_names.t_class.as_string_32 + " '" + l_stone.class_name + "'"
				a_menu.first.set_text (l_text)

				a_menu.extend (dev_window.commands.new_tab_cmd.new_menu_item)
				a_menu.last.set_text (interface_names.m_new_tab.as_string_32 + " '" + l_stone.class_name + "'")
				...
				a_menu.last.set_text (interface_names.m_new_window.as_string_32 + " '" + l_stone.class_name + "'")
				a_menu.last.select_actions.wipe_out
				...
				a_menu.last.set_text (interface_names.m_external_editor.as_string_32 + " '" + l_stone.class_name + "'")
				a_menu.last.select_actions.wipe_out
...
--				l_menu.extend (create {EV_MENU_ITEM}.make_with_text ("Move"))
			...
			elseif l_pebble_type.is_equal ({FEATURE_STONE}) then
				...
				l_text := interface_names.m_pick.as_string_32 + " " + interface_names.l_feature.as_string_32 + " '" + l_feature_stone.feature_name + "'"
				a_menu.first.set_text (l_text)


				a_menu.extend (dev_window.commands.new_tab_cmd.new_menu_item)
				a_menu.last.set_text (interface_names.m_new_tab.as_string_32 + " '" + l_feature_stone.feature_name + "'")
				a_menu.last.select_actions.wipe_out
				a_menu.last.select_actions.extend (agent (dev_window.commands.new_tab_cmd).execute_with_stone (l_stone))

				a_menu.extend (dev_window.new_development_window_cmd.new_menu_item)
				a_menu.last.set_text (interface_names.m_new_window.as_string_32 + " '" + l_feature_stone.feature_name + "'")
				a_menu.last.select_actions.wipe_out
				a_menu.last.select_actions.extend (agent (dev_window.new_development_window_cmd).execute_with_stone (l_stone))

				a_menu.extend (dev_window.commands.shell_cmd.new_menu_item)
				a_menu.last.set_text (interface_names.m_external_editor.as_string_32 + " '" + l_feature_stone.feature_name + "'")
				a_menu.last.select_actions.wipe_out
				a_menu.last.select_actions.extend (agent (dev_window.commands.shell_cmd).execute_with_stone (l_stone))
				a_menu.extend (create {EV_MENU_SEPARATOR})
			...
			elseif l_pebble_type.is_equal ({CLUSTER_STONE}) then
				...
				l_text := interface_names.m_pick.as_string_32 + " " + interface_names.l_cluster.as_string_32 + " '" + l_cluster_stone.group.name + "'"
				a_menu.first.set_text (l_text)

				a_menu.extend (dev_window.commands.new_tab_cmd.new_menu_item)
				a_menu.last.set_text (interface_names.m_new_tab.as_string_32 + " '" + l_cluster_stone.group.name + "'")
				a_menu.last.select_actions.wipe_out
				a_menu.last.select_actions.extend (agent (dev_window.commands.new_tab_cmd).execute_with_stone (l_stone))

				a_menu.extend (dev_window.new_development_window_cmd.new_menu_item)
				a_menu.last.set_text (interface_names.m_new_window.as_string_32 + " '" + l_cluster_stone.group.name + "'")
				a_menu.last.select_actions.wipe_out
				a_menu.last.select_actions.extend (agent (dev_window.new_development_window_cmd).execute_with_stone (l_stone))
