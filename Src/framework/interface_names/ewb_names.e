...
feature -- Access
	...
	arrow_class_name: STRING_32 							do Result := locale.translation ("--> Class name: ") end
	arrow_feature_name: STRING_32 						do Result := locale.translation ("--> Feature name: ") end
	arrow_filter_name: STRING_32 						do Result := locale.translation ("--> Filter name: ") end
	arrow_profile_infomation_file_name: STRING_32 		do Result := locale.translation ("--> Profile information file name (default: `profinfo'): ") end
	arrow_compile_type: STRING_32 						do Result := locale.translation ("--> Compile type (default: `workbench'): ") end
	arrow_used_profiler: STRING_32 						do Result := locale.translation ("--> Used profiler (default: `eiffel'): ") end
	arrow_subquery: STRING_32							do Result := locale.translation ("--> Subquery: ") end
	arrow_arguments: STRING_32							do Result := locale.translation ("--> Arguments: ") end
	arrow_operator_index_followed_by: STRING_32			do Result := locale.translation ("--> Operator index followed by operator ('and' or 'or'): ") end
	arrow_please_enter_an_operator_index: STRING_32 		do Result := locale.translation ("--> Please enter an operator index followed by an operator ('and' or 'or'): ") end
	arrow_operator_index: STRING_32						do Result := locale.translation ("--> Operator index: ") end
	arrow_new_operator: STRING_32						do Result := locale.translation ("--> New operator: ") end
	arrow_keep_assertions: STRING_32						do Result := locale.translation ("--> Keep assertions") end
	arrow_subquery_index: STRING_32						do Result := locale.translation ("--> Subquery index: ") end
	arrow_file_name: STRING_32							do Result := locale.translation ("--> File name: ") end
	arrow_filenames: STRING_32							do Result := locale.translation ("--> Filename(s): ") end
	command_arrow: STRING_32								do Result := locale.translation ("Command => ") end
	want_to_update_new_compiler: STRING_32 				do Result := locale.translation ("Do you want to update to new version of compiler?") end
	enter_name_for_configuration_file: STRING_32 		do Result := locale.translation ("Enter name for configuration file: ") end
	project_contains_no_compilable_target: STRING_32 	do Result := locale.translation ("This project contains no compilable target.") end
	has_more_than_one_target: STRING_32 					do Result := locale.translation ("This project has more than one target: ") end
	can_not_choose_a_target: STRING_32 					do Result := locale.translation ("You cannot choose a target because of the -stop/-batch option.") end
	select_the_target_you_want: STRING_32 				do Result := locale.translation ("Select the target you want (0 to quit): ") end
	invalid_target: STRING_32 							do Result := locale.translation ("Invalid target, select the target you want: ") end
	cannot_choose_name_because_of: STRING_32 			do Result := locale.translation ("You cannot choose the project location%Nbecause of the -stop/-batch option.%N") end
	...
	all_calees: STRING_32								do Result := locale.translation ("All callees") end
	only_assignees: STRING_32							do Result := locale.translation ("Only assignees") end
	only_creators: STRING_32								do Result := locale.translation ("Only creators") end
	index_must_be_an_integer: STRING_32					do Result := locale.translation ("Index must be an integer.%N") end
	operator_must_be: STRING_32							do Result := locale.translation ("Operator must be 'and' or 'or'.%N") end
	there_is_no_item_available: STRING_32				do Result := locale.translation ("There is no items available at this index.%N") end
	index_must_be_valid: STRING_32						do Result := locale.translation ("Index must be valid.%N") end
	file_name: STRING_32									do Result := locale.translation ("File name: ") end
	file_name_with_default: STRING_32					do Result := locale.translation ("File name (`Ace.ace' is the default): ") end
	invalid_choices: STRING_32							do Result := locale.translation ("Invalid choice%N%N") end
	include_parents: STRING_32							do Result := locale.translation ("Include parents") end
	ready_for_queries: STRING_32							do Result := locale.translation ("Ready for queries...") end
	an_error_occurred_while_loading_profiler: STRING_32	do Result := locale.translation ("An error occurred while loading the configuration for your profiler.") end
	please_check_with_your_system_administator: STRING_32 do Result := locale.translation ("Please check with your system administrator whether your profiler is supported.") end
	file_already_exists: STRING_32						do Result := locale.translation ("File already exists.%N") end
	no_active_queries: STRING_32							do Result := locale.translation ("No active queries") end
	you_should_first_manipulate_the_subqueries: STRING_32 do Result := locale.translation ("You should first manipulate the subqueries") end
	query: STRING_32										do Result := locale.translation ("Query:") end
	all_senders: STRING_32								do Result := locale.translation ("All senders") end
	only_assigners: STRING_32							do Result := locale.translation ("Only assigners") end
	all_subqueries: STRING_32							do Result := locale.translation ("All subqueries:%N") end
	the_total_active_query: STRING_32					do Result := locale.translation ("%NThe total active query:%N") end
	entry_disabled: STRING_32							do Result := locale.translation ("  -> Entry disabled %N") end

	no_value_entered: STRING_32
	...
	a_subquery_has_the_following_form: STRING_32
	...
	specify_the_ace_file: STRING_32
...
feature -- Errors
	...
	err_press_return_to_resume: STRING_32 					do Result := locale.translation ("Press <Return> to resume compilation or <Q> to quit") end
	err_too_many_arguments: STRING_32 						do Result := locale.translation ("Too many arguments. The following arguments will be ignored:") end
	session_aborted: STRING_32 								do Result := locale.translation ("ISE Eiffel: Session aborted%N") end
	exception_tag: STRING_32 								do Result := locale.translation ("Exception tag: ") end
	cannot_create_file: STRING_32 							do Result := locale.translation ("Cannot create file: ") end
	several_class_has_the_same_name: STRING_32 				do Result := locale.translation ("Several classes have the same name:%N") end
	read_only_project_cannot_compile: STRING_32 				do Result := locale.translation ("Read-only project: cannot compile.%N") end
	read_only_project_no_c_code_to_compile: STRING_32 		do Result := locale.translation ("Read-only project: no C code to compile in F_code.%N") end
