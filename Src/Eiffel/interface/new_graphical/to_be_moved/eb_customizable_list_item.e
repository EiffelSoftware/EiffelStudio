...
feature -- Initialization
	...
	make (v: EB_TOOLBARABLE)
		...
		do
			...
			set_pixmap (v.pixmap)
			data := v
			if attached {EB_TOOLBARABLE_COMMAND} v as command then
				is_separator := False
			else
				is_separator := True
			end -- if
...
feature -- Access
	...
	custom_parent: detachable EB_CUSTOM_TOOLBAR_LIST
			-- Convert `parent' into a EB_CUSTOM_TOOLBAR_LIST
		do
			if attached {EB_CUSTOM_TOOLBAR_LIST} parent as res then
				Result := res
			end
		end
