...
feature{MYSQLI_EXTERNALS} -- Internals
	...
	bind (a_bind: POINTER; a_buffer, a_is_null, a_length: MANAGED_POINTER)
		...
		do
			c_struct_mysql_bind_set_buffer_type (a_bind, MYSQL_TYPE_NULL)
--			c_struct_mysql_bind_set_is_null (a_bind, a_length.item)
		end
...
feature -- Access (Date and Time Types)
	...
	as_date_and_time_tuple: TUPLE [years, months, days, hours, minutes, seconds: INTEGER_32]
		...
		do
			...
			raise ("MYSQLI_NULL_VALUE.as_date_and_time_tuple: current 'value' is not representable as a date and time tuple.")
		end
	...
	as_timestamp: DATE_TIME
		...
		do
			...
			raise ("MYSQLI_NULL_VALUE.as_timestamp: current 'value' is not representable as a timestamp.")
		end
	...
	as_date: DATE
		...
		do
			create Result.make_now
			raise ("MYSQLI_NULL_VALUE.as_date: current 'value' is not representable as a date.")
		end
	...
	as_time: TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_NULL_VALUE.as_time: current 'value' is not representable as a time.")
		end
	...
	as_year: INTEGER
		...
		do
			raise ("MYSQLI_NULL_VALUE.as_year: current 'value' is not representable as a year.")
		end
...
feature -- Access (Set Type)
	...
	as_set: SET [STRING]
		...
		do
			...
			Result := l_linked_set
			raise ("MYSQLI_NULL_VALUE.as_set: current 'value' is not representable as a set.")
		end
