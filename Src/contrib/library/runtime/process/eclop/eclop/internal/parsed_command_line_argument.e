...
feature {COMMAND_LINE_PARSER} -- Access
	...
	invalidly_grouped_option_names: LIST [STRING]
	...
	parse_short_option_names
		...
		do
			...
			if argument.count >= 3 then
				if specified_options.has (s) then
					os := specified_options @ s
	...
	synonym_short_name (s: STRING): STRING
		...
		do
			...
			loop
				...
				if os.has_long_name and then os.long_name.is_equal (s) then
					found := True
					if os.has_short_name then
						Result := os.short_name
					end
				end
