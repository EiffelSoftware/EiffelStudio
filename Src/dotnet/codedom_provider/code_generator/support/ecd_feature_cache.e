	...
	add_feature (a_type_name: STRING; an_eiffel_feature_name: STRING) is
		...
		do
			if not is_generated_type (a_type_name) then
				features.put (a_type_name, (create {ECD_ENTITY_NAME_RESOLVER}).unique_entity_name (an_eiffel_feature_name))
			else
				features.put (a_type_name, an_eiffel_feature_name)
			end
		end

	initialize_features (a_type: ECD_GENERATED_TYPE) is
		...
		local
			l_attribute: ECD_ATTRIBUTE
			l_function: ECD_FUNCTION
			l_get_property: ECD_PROPERTY_GETTER
			l_set_property: ECD_PROPERTY_SETTER
			l_snippet_feature: ECD_SNIPPET_FEATURE
