note
	description: "Criterion to test if top or bottom indexing clause of a class has an indexing with a given tag"
		...
		do
			...
			if l_class_as /= Void then
				if l_class_as.top_indexes /= Void then
					...
					if not Result then
						if l_class_as.bottom_indexes /= Void then
							Result := indexing_clause_has_tag (l_class_as.bottom_indexes)
						end
					end
				end
			end
