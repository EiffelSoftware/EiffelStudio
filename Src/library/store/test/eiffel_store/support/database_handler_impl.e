note
	description: "Summary description for {DATABASE_HANDLER}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
	...
	execute_reader
		...
		do
			if not l_retried then
				...
				io.put_string ( generator+".execute_reader Successful")
			end
		rescue
	...
	execute_writer
		...
		do
		    if not  l_retried then
				...
				io.put_string ( generator+".execute_writer Successful")
			end
		rescue
			if is_connected then
				disconnect
