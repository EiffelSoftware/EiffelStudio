...
void ecom_control_library::IServiceProvider_impl_proxy::ccom_remote_query_service(  /* [in] */ GUID * guid_service,  /* [in] */ GUID * riid,  /* [out] */ EIF_OBJECT ppv_object )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IServiceProvider->RemoteQueryService(guid_service,riid,tmp_ppv_object);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
