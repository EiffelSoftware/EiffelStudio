...
feature {ECD_CONSUMER_FACTORY} -- Visitor features.
	...
	generate_type (a_source: SYSTEM_DLL_CODE_TYPE_DECLARATION) is
			...
			-- | Initialize the ECD_TYPE instance with `a_source' -> Call `Initialize_type'
		...
		do
			...
			if l_deferred then
				create {ECD_DEFERRED_TYPE} l_type.make
			end
			if l_frozen then
				if l_expanded then
					create {ECD_FROZEN_EXPANDED_TYPE} l_type.make
				else
					create {ECD_FROZEN_TYPE} l_type.make
				end
			elseif l_expanded then
				create {ECD_EXPANDED_TYPE} l_type.make
			else
				create {ECD_GENERATED_TYPE} l_type.make
			end
...
feature {NONE} -- Type generation
	...
	initialize_type (a_source: SYSTEM_DLL_CODE_TYPE_DECLARATION; a_type: ECD_GENERATED_TYPE) is
	...
	initialize_type_without_parents (a_source: SYSTEM_DLL_CODE_TYPE_DECLARATION; a_type: ECD_GENERATED_TYPE) is
	...
	initialize_type_parents (a_source: SYSTEM_DLL_CODE_TYPE_DECLARATION; a_type: ECD_GENERATED_TYPE) is
			...
			if l_parents /= Void then
				...
				loop
					...
					if not l_interface_type then
						...
						l_object_parent.add_undefine_clause (create {ECD_UNDEFINE_CLAUSE}.make ("finalize"))
						l_interface_type := True
					end
					l_object_parent.add_undefine_clause (create {ECD_UNDEFINE_CLAUSE}.make ("get_hash_code"))
					l_object_parent.add_undefine_clause (create {ECD_UNDEFINE_CLAUSE}.make ("equals"))
					l_object_parent.add_undefine_clause (create {ECD_UNDEFINE_CLAUSE}.make ("to_string"))
	...
	initialize_type_features (a_source: SYSTEM_DLL_CODE_TYPE_DECLARATION; a_type: ECD_GENERATED_TYPE) is
	...
	initialize_indexing_clause (a_source: SYSTEM_DLL_CODE_TYPE_DECLARATION; a_type: ECD_GENERATED_TYPE) is
