...
--			my_window.set_style (feature {WINFORMS_CONTROL_STYLES}.opaque, True)

			-- Make sure form redraws when it is resized
--			my_window.set_style (feature {WINFORMS_CONTROL_STYLES}.resize_redraw, True)
...
--			create l_text_image.make_from_original (create {DRAWING_BITMAP}.make_from_stream (feature {ASSEMBLY}.get_executing_assembly.get_manifest_resource_stream (("marble.jpg").to_cil)))
			...
			my_window.set_font (create {DRAWING_FONT}.make_from_family_and_em_size (serif_font_family, 20))
			...
			create title_shadow_brush.make (feature {DRAWING_COLOR}.from_argb_integer_32_color (70, feature {DRAWING_COLOR}.get_black))

			-- Set up fonts and brushes for printing japanese text
--			try {
--				japanese_font create {DRAWING_FONT}.make ("MS Mincho", 36)
--				linear_grad_brush = new LinearGradientBrush(new Point(0, 0), new Point(0, 45), feature {DRAWING_COLOR}.get_Blue, feature {DRAWING_COLOR}.get_Red)
--			} catch (Exception ex) {
...
feature -- Implementation
	...
	on_paint (e: WINFORMS_PAINT_EVENT_ARGS) is
		...
		local
			...
			dummy: SYSTEM_OBJECT
		do
			graph := e.get_graphics

			e.get_graphics.set_smoothing_mode (feature {DRAWING_SMOOTHING_MODE}.anti_alias)

			-- Fill the background use the texture brush
			-- and then apply a white wash
			graph.fill_rectangle (background_brush, my_window.get_client_rectangle)
			graph.fill_rectangle (create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.from_argb_integer_32_color (180, feature {DRAWING_COLOR}.get_white)), my_window.get_client_rectangle)

			-- Simple draw hello world
			graph.draw_string_string_font_brush_real_real (("Hello World").to_cil, my_window.get_font, create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_black), 10, 10)

			-- Draw a textured string
			title_text := ("get_graphics  Samples").to_cil
			...
			window_center := my_window.get_display_rectangle.get_width / 2
			string_size := e.get_graphics.measure_string (text_to_draw, text_font)
			start_pos := window_center - (string_size.get_width / 2)
			graph.draw_string_string_font_brush_real_real (text_to_draw, text_font, create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_red), start_pos, 10)

			-- Now draw a string flowed into a rectangle
			rectangle_11.make_with_x (20, 150, 250, 300)
			rectangle_12.make_with_x (20, 150, 250, 300)
			graph.fill_rectangle (create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_gainsboro), rectangle_12)
			graph.draw_string (flowed_text_1, text_font, create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_blue), rectangle_11)
			...
			graph.fill_rectangle (create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_gainsboro), rectangle_22)
			create format.make
			format.set_alignment (feature {DRAWING_STRING_ALIGNMENT}.center)
			graph.draw_string_string_font_brush_rectangle_f_string_format (flowed_text_2, text_font, create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_Blue), rectangle_21, format)

			-- Work out how many lines and characters we printed just now
--			dummy := e.get_graphics.measure_string_string_font_size_f_string_format_integer_32 (flowed_text_2, text_font, rectangle_21.get_size, format, characters, lines)
...
--			e.get_graphics.draw_string (what_rendered_text, text_font, create {DRAWING_SOLID_BRUSH}.make (feature {DRAWING_COLOR}.get_Black), 400,440)
...
--			Precursor.dispose (disposing)
	...
	initialize_component is
		...
		do
			...
			my_window.set_size_size (l_size)
			my_window.set_text (("GDI+ Text Samples").to_cil)
		end
