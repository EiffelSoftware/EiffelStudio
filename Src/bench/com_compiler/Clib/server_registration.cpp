	...
	-----------------------------------------------------------*/
{
	...
	if (!(CEiffelProject_cls_object.IsInitialized))
		...
		hr = CoRegisterClassObject (CLSID_CEiffelProject_, static_cast<IClassFactory*>(&CEiffelProject_cls_object), CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE, &dwRegister_CEiffelProject);
	if (FAILED (hr))
	{
		com_eraise (f.c_format_message (hr), HRESULT_CODE (hr));
	}
	if (!(CEiffelCompiler_cls_object.IsInitialized))
		...
		hr = CoRegisterClassObject (CLSID_CEiffelCompiler_, static_cast<IClassFactory*>(&CEiffelCompiler_cls_object), CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE, &dwRegister_CEiffelCompiler);
	if (FAILED (hr))
	{
		com_eraise (f.c_format_message (hr), HRESULT_CODE (hr));
	}
