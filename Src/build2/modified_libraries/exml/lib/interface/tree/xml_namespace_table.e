...
inherit
   DS_HASH_TABLE [UCSTRING, UCSTRING]
      rename
...
feature {ANY} -- 
   ...
   override_with_list (l: DS_BILINEAR [XML_NAMESPACE]) is
	 ...
	 l_not_void: l /= Void
      local
	 cs: DS_BILINEAR_CURSOR [XML_NAMESPACE]
      do
	 from
	    cs := l.new_cursor
	    cs.start
...
feature {NONE} -- Implementation
   uc_empty: UCSTRING is
      once
	 create Result.make_from_string ("")
      end
end -- class XML_NAMESPACE_TABLE
--|-------------------------------------------------------------------------
