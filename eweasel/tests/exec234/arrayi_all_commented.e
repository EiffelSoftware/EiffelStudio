...
--						  (agent (l_max_index: INTEGER): INTEGER do Result := l_max_index + 1 end).item([max_index])
		do
--			(agent (a_min_index, a_max_index: INTEGER) do
--				lower := a_min_index
--				upper := a_max_index
--				if a_min_index <= a_max_index then
--					(agent (n: INTEGER) do make_area (n) end (a_max_index - a_min_index + 1)).call ([])
--				else
--					(agent do make_area (0) end).call ([])
--				end
--			end (min_index, max_index)).call ([])
--		ensure
...
--									end (ll_i)).item ([])
--							end).item ([i])
--						/= Void and then v.is_equal (item (i))
--					then
--						Result := Result + 1
--					end
					i := i + 1
