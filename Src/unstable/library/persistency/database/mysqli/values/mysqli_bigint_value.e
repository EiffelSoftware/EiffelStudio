note
	description: "MySQL BIGINT Value"
	author: "haroth@student.ethz.ch"
	date: "$Date: 2012-03-16 13:50:04 +0400$"
	revision: "$Revision$"
...
feature{NONE} -- Initialization
	...
	make (a_value: INTEGER_64)
		...
		do
			value := a_value.to_natural_64
		end

	make_as_integer_64 (a_value: INTEGER_64)
			-- Set `value' to `a_value'.
		do
			value := a_value.to_natural_64
		end

	make_as_natural_64 (a_value: NATURAL_64)
			-- Set `value' to `a_value'.
		do
			value := a_value
			is_unsigned := True
...
feature -- Access (Types)
	...
	as_string_32: STRING_32
		...
		do
			Result := as_integer_64.out.to_string_32
		end
...
feature -- Access (Date and Time Types)
	...
	as_date_and_time_tuple: TUPLE [years, months, days, hours, minutes, seconds: INTEGER_32]
		...
		do
			...
			raise ("MYSQLI_ENUM_VALUE.as_date_and_time_tuple: current 'value' is not representable as a date and time tuple.")
		end
	...
	as_timestamp: DATE_TIME
		...
		do
			...
			raise ("MYSQLI_BIGINT_VALUE.as_timestamp: current 'value' is not representable as a timestamp.")
		end
	...
	as_date: DATE
		...
		do
			create Result.make_now
			raise ("MYSQLI_BIGINT_VALUE.as_date: current 'value' is not representable as a date.")
		end
	...
	as_time: TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_BIGINT_VALUE.as_time: current 'value' is not representable as a time.")
		end
	...
	as_datetime: DATE_TIME
		...
		do
			create Result.make_now
			raise ("MYSQLI_BIGINT_VALUE.as_datetime: current 'value' is not representable as a datetime.")
		end

	as_year: INTEGER
			-- This value as a INTEGER
		do
			raise ("MYSQLI_BIGINT_VALUE.as_year: current 'value' is not representable as a year.")
		end
...
feature -- Access (Set Type)
	...
	as_set: SET [STRING]
		...
		do
			...
			Result := l_linked_set
			raise ("MYSQLI_BIGINT_VALUE.as_set: current 'value' is not representable as a set.")
		end
