note
	description:
		"[
			A stress test for passive processors in SCOOP.

			The test is very similar to test#bench016. It is designed to put a lot ofpressure on the memory
			allocationsubsystem (and consequently GC) while keeping the threads busy. To a lesser extent this
			test also exercises the evaluation of wait conditions.
			...
			At the end of a task's lifecycle a validator processor checks the correctness of the computation.
		]"
...
feature -- Constants
	...
	default_task_count: INTEGER = 100
...
feature {NONE} -- Implementation
	...
	new_task: separate TASK
		...
		do
			...
			create <NONE> Result.make (operation_count, size)
		end
