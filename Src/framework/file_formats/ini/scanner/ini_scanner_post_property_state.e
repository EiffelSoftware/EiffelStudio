note
	description: "State scanner state when scanner passed a token consituting a property name."
...
feature -- Basic Operations
	...
	scan (a_line: STRING; a_index: INTEGER)
		...
		do
			...
			c := a_line.item (1)
			if c = {INI_SCANNER_SYMBOLS}.assigner_indicator then
				...
				l_next_state := state_pool.state ({INI_SCANNER_POST_ASSIGNER_STATE})
					...
					-- Unknown token
				set_token_info (a_line, {INI_SCANNER_TOKEN_TYPE}.unknown, a_index)
			end
...
end -- class {INI_SCANNER_POST_PROPERTY_STATE}
