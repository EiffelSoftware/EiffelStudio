...
feature -- Factory
	...
	criteria (a_name: READABLE_STRING_32; a_value: READABLE_STRING_32): detachable SCORER_CRITERIA [G]
		local
			l_is_not: BOOLEAN
	...
	default_criteria (a_value: READABLE_STRING_32): detachable SCORER_CRITERIA [G]
		do
			if
...
feature {NONE} -- Parse
			...
			if tok.is_group and then attached tok.group as group_tok then
				...
				if f_right /= Void then
					...
					if l_op.is_operator_not then
						create {SCORER_CRITERIA_NOT [G]} Result.make (f_right)
					else
						if
	...
	criteria_tokens (s: READABLE_STRING_32): detachable ARRAYED_LIST [SCORE_TOKEN]
		require
			s_attached: s /= Void
		local
			c: CHARACTER_32
