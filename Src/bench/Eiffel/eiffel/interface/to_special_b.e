indexing
	description: "Compiled class TO_SPECIAL"
	date: "$Date$"
	revision: "$Revision$"
	...
	check_validity is
		...
		do
			...
			if generics = Void or else generics.count /= 1 then
				create special_error.make (Case_1, Current)
			...
			then
				create special_error.make (Case_2, Current)
				Error_handler.insert_error (special_error)
			end
			...
			then
				create special_error.make (Case_3, Current)
				Error_handler.insert_error (special_error)
			end
		end
