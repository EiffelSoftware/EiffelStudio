...
feature -- Access
	...
	observers: !DS_ARRAYED_LIST [!EIFFEL_TEST_CAPTURE_OBSERVER]
	...
	capture_call_stack_element (a_cse: !EIFFEL_CALL_STACK_ELEMENT)
		...
		do
			if {l_feature: !E_FEATURE} a_cse.routine then
				...
				if not l_abort then
					observers.do_all (agent {!EIFFEL_TEST_CAPTURE_OBSERVER}.on_invocation_capture (l_element))
				end
			end
		ensure
			capturing_invocations: is_capturing_invocations
...
feature {NONE} -- Basic operations
	...
	process_next_object
			...
			--    appropriate {EIFFEL_TEST_CAPTURED_OBJECT} and notify observers. Remove item from
			--    `object_queue'.
		require
		...
		local
			...
			l_type: !STRING
			l_object: !EIFFEL_TEST_CAPTURED_OBJECT
		do
			...
			if {l_class: !EIFFEL_CLASS_C} l_adv.dynamic_class then
				...
				if l_system.string_32_class = l_classi or l_system.string_8_class = l_classi then
					if {l_string: !STRING} l_adv.dump_value.truncated_string_representation (0, -1).to_string_8 then
						create {EIFFEL_TEST_CAPTURED_STRING_OBJECT} l_object.make (l_id, l_type, l_string)
					else
						create {EIFFEL_TEST_CAPTURED_STRING_OBJECT} l_object.make (l_id, l_type, "")
					end
...
--					create {EIFFEL_TEST_CAPTURED_ITEM_OBJECT} l_object.make (l_id, l_type, 0)
				else
					if l_class.is_special or l_class.is_tuple then
						create {EIFFEL_TEST_CAPTURED_ITEM_OBJECT} l_object.make (l_id, l_type, l_adv.children.count)
					else
						create {EIFFEL_TEST_CAPTURED_ATTRIBUTE_OBJECT} l_object.make (l_id, l_type, l_adv.children.count)
					end
			...
			observers.do_all (agent {!EIFFEL_TEST_CAPTURE_OBSERVER}.on_object_capture (l_object))
		end

	fill_object (a_object: !EIFFEL_TEST_CAPTURED_OBJECT; a_list: !DS_LINEAR [!ABSTRACT_DEBUG_VALUE]; a_depth: NATURAL)
...
feature {NONE} -- Events
	...
	on_prepare
			-- <Precursor>
		do
			observers.do_all (agent (a_obs: !EIFFEL_TEST_CAPTURE_OBSERVER)
				do
