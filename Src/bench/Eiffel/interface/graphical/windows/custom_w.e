		...
		do
			...
			separator8.set_horizontal (false);
				...
				!!export_spec.make (new_name, global_form);
					...
					!!export_hole.special_make (export_spec, Current,export_choice, drop_export_choice);
...
!!dummy_choice.make ("FFOOFF", origin_choice);
					...
					!!origin_hole.special_make (origin_spec, Current,origin_choice, drop_origin_choice);
...
--				origin_hole.receive (System.class_of_id (local_origin))
			...
			if user_format.dos then do_tog.arm else do_tog.disarm end;
			if user_format.onces then once_tog.arm else once_tog.disarm end;
			if user_format.deferreds then deferred_tog.arm else deferred_tog.disarm end;
			if user_format.externals then external_tog.arm else external_tog.disarm end;
			if user_format.attributes then attributes_tog.arm else attributes_tog.disarm end;
			if user_format.constants then constants_tog.arm else constants_tog.disarm end;
			if user_format.procedures then procedures_tog.arm else procedures_tog.disarm end;
			if user_format.functions then functions_tog.arm else functions_tog.disarm end;
	...
	set_export_class_id (c: like export_class_id) is do export_class_id := c end;

	set_origin_class_id (c: like origin_class_id) is do origin_class_id := c end;
