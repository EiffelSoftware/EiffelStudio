...
feature -- Command
						...
						then
							...
							if l_parent_parent = Void then
								...
								if attached {SD_ZONE} Current as lt_zone and l_other /= Void then
									docking_manager.query.inner_container (lt_zone).save_spliter_position (l_other, generating_type.name + ".recover_normal_size_from_minimize")
								else
								...
								if l_other /= Void then
									if attached {SD_ZONE} Current as lt_zone_2 then
										docking_manager.query.inner_container (lt_zone_2).restore_spliter_position (l_other, generating_type.name + ".recover_normal_size_from_minimize")
									else
										check not_possible: False end
									end
								end
	...
	minimize
		...
		do
						...
						if attached {SD_ZONE} Current as lt_zone then
							if l_parent.first = Current then
								...
								if attached l_other then
									docking_manager.query.inner_container (lt_zone).save_spliter_position (l_other, generating_type.name + ".minimize")
								end
								l_parent.wipe_out
			...
			if l_other /= Void then
				if attached {SD_ZONE} Current as lt_zone_3 then
					docking_manager.query.inner_container (lt_zone_3).restore_spliter_position (l_other, generating_type.name + ".minimize")
				else
					check not_possible: False end
				end
			end
