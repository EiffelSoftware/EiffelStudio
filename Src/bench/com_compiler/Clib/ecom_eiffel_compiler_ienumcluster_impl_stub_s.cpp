...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_Eif_compiler.ccom_ce_pointed_cell_29 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_Eif_compiler.ccom_ec_pointed_cell_29 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
