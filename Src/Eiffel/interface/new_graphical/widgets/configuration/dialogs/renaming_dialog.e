...
inherit
	PROPERTY_DIALOG [CONF_HASH_TABLE [STRING, STRING]]
		redefine
			initialize,
			on_ok
		end
...
feature {NONE} -- Initialization
	...
	initialize is
		...
		do
			...
			create l_btn.make_with_text_and_action ("+", agent on_add)
			hb.extend (l_btn)
			hb.disable_item_expand (l_btn)
			l_btn.set_minimum_width (small_button_width)
			create l_btn.make_with_text_and_action ("-", agent on_remove)
			hb.extend (l_btn)
			hb.disable_item_expand (l_btn)
			l_btn.set_minimum_width (small_button_width)
