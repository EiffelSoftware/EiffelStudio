...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleInPlaceUIWindow->GetBorder(lprect_border);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleInPlaceUIWindow->RequestBorderSpace(pborderwidths);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IOleInPlaceUIWindow->SetBorderSpace(pborderwidths);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IOleInPlaceUIWindow_impl_proxy::ccom_set_active_object(  /* [in] */ ecom_control_library::IOleInPlaceActiveObject * p_active_object,  /* [in] */ EIF_OBJECT psz_obj_name )
