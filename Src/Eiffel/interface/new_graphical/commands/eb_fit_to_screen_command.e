	...
	execute
		...
		do
			if is_sensitive then
				...
				old_scale := (l_world.scale_factor * 100).truncated_to_integer.max (1)
				wc := tool.world_cell
				wc.fit_to_screen
				new_scale_factor := l_world.scale_factor
				new_scale := (new_scale_factor * 100).truncated_to_integer.max (1)
...
feature {NONE} -- Implementation
	...
	pixel_buffer: EV_PIXEL_BUFFER
		...
		do
			Result := pixmaps.icon_pixmaps.diagram_fit_to_screen_icon_buffer
		end
