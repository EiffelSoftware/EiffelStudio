note
	description: "[
		PUSH dialog, for now, only git, but in the future, it could be used for any distributed SCM.
	]"
	date: "$Date$"
	revision: "$Revision$"
...
--			f: EV_FRAME
		...
		do
			...
			set_button_text (dialog_buttons.ok_button, scm_names.button_push)
...
--			end
			...
			elseif l_remote.is_whitespace then
				...
				if l_use_external_terminal then
					...
					l_push.report_success ("Check the terminal for more information.")
				else
					scm_service.push (l_push)
				end
			end
			...
			if attached l_push.execution_message as m then
...
feature -- Access
	...
	title: STRING_32
			-- <Precursor>
		do
			Result := scm_names.title_scm_push
		end

	button_push: detachable EV_BUTTON
		do
