...
feature{QL_SHARED_SCOPES} -- Initialization
	...
	make is
			-- Initialize.
		do
			index := assertion_scope_index
		ensure
			scope_index_set: index = assertion_scope_index
		end
...
feature -- Access
	...
	path_domain_generator (a_item: QL_ASSERTION; a_path: STRING): QL_DOMAIN_GENERATOR is
			-- Domain generator for current scope
		do
	...
	empty_domain: QL_ASSERTION_DOMAIN is
			-- An empty domain whose scope is assertion
		do
			create Result.make
		end
...
feature -- Metric support
	...
	basic_scope (a_calculate_function: FUNCTION [ANY, TUPLE [QL_ASSERTION], DOUBLE]; a_criterion: QL_ASSERTION_CRITERION): QL_METRIC_ASSERTION_BASIC_SCOPE_INFO is
