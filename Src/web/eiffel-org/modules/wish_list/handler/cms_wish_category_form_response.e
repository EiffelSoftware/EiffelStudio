note
	description: "Summary description for {CMS_WISH_CATEGORY_FORM_RESPONSE}."
	date: "$Date$"
	revision: "$Revision$"

class
	CMS_WISH_CATEGORY_FORM_RESPONSE
...
feature -- Process
	...
	process
		...
		do
			...
			if
				cid > 0 and then
				attached wish_api.category_by_id (cid) as l_category
			then
				if
...
feature -- Process Edit
	...
	edit_form (a_category: CMS_WISH_LIST_CATEGORY)
		local
		...
		do
			...
			if a_category.has_id then
				add_to_menu (create {CMS_LOCAL_LINK}.make (translation ("View", Void),"resources/wish/category/" + a_category.id.out), primary_tabs)
				add_to_menu (create {CMS_LOCAL_LINK}.make (translation ("Edit", Void),"resources/wish/category/" + a_category.id.out + "/edit"), primary_tabs)
...
feature -- Process Delete
	...
	delete_form (a_category: CMS_WISH_LIST_CATEGORY)
		local
...
feature -- Process New
	...
	new_form
		local
			...
			b: STRING
			fd: detachable WSF_FORM_DATA
			l_category: detachable CMS_WISH_LIST_CATEGORY
		do
			create b.make_empty
...
feature -- Form	
	...
	edit_form_submit (fd: WSF_FORM_DATA; a_category: detachable CMS_WISH_LIST_CATEGORY; b: STRING)
		local
		...
		do
			...
			l_update_category := attached {WSF_STRING} fd.item ("op") as l_op and then l_op.same_string ("Update wish category")
			if l_update_category then
				...
				if a_category /= Void then
					...
					if l_category.has_id then
						create {CMS_LOCAL_LINK} lnk.make (translation ("View", Void),"resources/wish/category/" + l_category.id.out )
			...
			l_save_category := attached {WSF_STRING} fd.item ("op") as l_op and then l_op.same_string ("Create wish category")
	...
	new_edit_form (a_category: detachable CMS_WISH_LIST_CATEGORY; a_url: READABLE_STRING_8; a_name: STRING): CMS_FORM
	...
	new_form_validate (fd: WSF_FORM_DATA; b: STRING)
		do
			if attached fd.string_item ("op") as f_op then
				if
					f_op.is_case_insensitive_equal_general ("Create wish category")
				then
					if attached fd.string_item ("synopsis") as l_synopsis then
						if attached wish_api.category_by_name (l_synopsis) then
							fd.report_invalid_field ("synopsis", "Category already taken!")
						end
					else
						fd.report_invalid_field ("synopsis", "missing synopsis")
					end
				elseif	f_op.is_case_insensitive_equal_general ("Update wish category")
				then
					if attached fd.string_item ("synopsis") as l_synopsis then
						if attached  {CMS_WISH_LIST_CATEGORY} wish_api.category_by_name (l_synopsis) as ll_synop and then
	...
	new_delete_form (a_category: detachable CMS_WISH_LIST_CATEGORY; a_url: READABLE_STRING_8; a_name: STRING;): CMS_FORM
		...
		do
			...
			then
				...
				ts.set_formaction ("/resources/wish/category/" + a_category.id.out)
				f.extend (ts)
			end
	...
	populate_form (a_form: WSF_FORM; a_category: detachable CMS_WISH_LIST_CATEGORY)
		...
		do
			if a_category /= Void then
				create fs.make
				fs.set_legend ("Wish Category Information")
				a_form.extend (fs)
				...
				create ts.make ("op")
				ts.set_default_value ("Update wish category")
				a_form.extend (ts)
			else
				create fs.make
				fs.set_legend ("New Wish Category Information")
				a_form.extend (fs)
				a_form.extend_html_text ("<br/>")
				...
				create ts.make ("op")
				ts.set_default_value ("Create wish category")
				a_form.extend (ts)
			end
		end

	change_category (a_form_data: WSF_FORM_DATA; a_category: CMS_WISH_LIST_CATEGORY)
		...
		do
			if attached a_form_data.string_item ("op") as f_op then
				if f_op.is_case_insensitive_equal_general ("Update wish category") then
					if
						attached a_form_data.string_item ("category-id") as l_category_id and then
						attached {CMS_WISH_LIST_CATEGORY} wish_api.category_by_id (l_category_id.to_integer_64)
					then
						if
						...
						then
							...
							wish_api.save_category (a_category)
						end
					end
				end
			end
	...
	create_category (a_form_data: WSF_FORM_DATA)
		local
			c: CMS_WISH_LIST_CATEGORY
		do
			if attached a_form_data.string_item ("op") as f_op then
				if f_op.is_case_insensitive_equal_general ("Create wish category") then
					if
						attached  a_form_data.string_item ("synopsis") as l_synopsis
					then
						create c.make_empty
						...
						wish_api.save_category (c)

						if wish_api.has_error then
							-- handle error
						else
							add_success_message ("Created wish category")
						end
