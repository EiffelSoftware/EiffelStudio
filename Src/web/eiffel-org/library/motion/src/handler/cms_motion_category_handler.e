note
	description: "Summary description for {CMS_MOTION_CATEGORY_HANDLER}."
	date: "$Date$"
	revision: "$Revision$"

class
	CMS_MOTION_CATEGORY_HANDLER
...
feature -- Query
	...
	category_id_path_parameter (req: WSF_REQUEST): INTEGER_64
			-- Cateogory id passed as path parameter for request `req'.
		local
			s: STRING
		do
...
feature -- HTTP Methods
	...
	do_get (req: WSF_REQUEST; res: WSF_RESPONSE)
		...
		local
			...
			l_uid: INTEGER_64
			edit_response: CMS_MOTION_CATEGORY_FORM_RESPONSE
			view_response: CMS_MOTION_CATEGORY_VIEW_RESPONSE
			r: CMS_RESPONSE
		do
			create {FORBIDDEN_ERROR_CMS_RESPONSE} r.make (req, res, api)
			if r.has_permission ("admin " + motion_api.item + " category") then
				if req.percent_encoded_path_info.ends_with_general ("/edit") then
					check valid_url: req.percent_encoded_path_info.starts_with_general ("/" + motion_api.resource_path + "/" + motion_api.item + "/category/") end
					create edit_response.make (req, res, api, motion_api)
				...
				elseif req.percent_encoded_path_info.ends_with_general ("/delete")  then
					...
					l_uid := category_id_path_parameter (req)
					if l_uid > 0 then
						l_category := motion_api.category_by_id (l_uid)
						if
							l_category /= Void
						then
							create view_response.make (req, res, api, motion_api)
							view_response.execute
						else
							send_not_found (req, res)
						end
					else
						create_new_category (req, res)
					end
				end
			else
				r.execute
	...
	do_post (req: WSF_REQUEST; res: WSF_RESPONSE)
		...
		do
			...
			if r.has_permission ("admin " + motion_api.item + " category") then
				...
				elseif req.percent_encoded_path_info.ends_with_general ("/add/category") then
					create edit_response.make (req, res, api, motion_api)
					edit_response.execute
				end
			else
...
feature {NONE} -- New Category

	create_new_category (req: WSF_REQUEST; res: WSF_RESPONSE)
		local
			edit_response: CMS_MOTION_CATEGORY_FORM_RESPONSE
		do
			if req.percent_encoded_path_info.starts_with ("/" + motion_api.resource_path + "/" + motion_api.item + "/add/category") then
				create edit_response.make (req, res, api, motion_api)
				edit_response.execute
			else
