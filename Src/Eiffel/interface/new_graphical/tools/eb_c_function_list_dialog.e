...
feature {NONE} -- Implementation
	...
	rows: ARRAYED_LIST [TUPLE [EV_GRID_ITEM, EV_GRID_ITEM]]
...
feature {NONE} -- Grid binding
	...
	fill_rows
		...
		do
			...
			until
				l_file_table.after
			loop
				l_rows.extend ([target_grid_item (l_file_table.item_for_iteration), open_file_grid_item (l_file_table.key_for_iteration)])
				l_file_table.forth
			end
		end
	...
	bind_grid
			-- Bind `grid' with `rows'.
		local
			l_cursor: INDEXABLE_ITERATION_CURSOR [TUPLE [a_target_item: EV_GRID_ITEM; a_open_dialog_item: EV_GRID_ITEM]]
...
feature{NONE} -- Implementation/Sorting
	...
	sort_agent (a_column_list: LIST [INTEGER_32]; a_comparator: AGENT_LIST_COMPARATOR [TUPLE [EV_GRID_ITEM, EV_GRID_ITEM]])
		...
		local
			l_sorter: QUICK_SORTER [TUPLE [EV_GRID_ITEM, EV_GRID_ITEM]]
		do
