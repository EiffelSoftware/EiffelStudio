...
feature -- Measurrment
	...
	item_found_count: INTEGER is
		...
		do
			Result := item_founded.integer_32_item (2)
		end	
...
feature -- Status report
	...
	found_type (a_type_name: STRING): BOOLEAN is
		...
		do
			...
			loop
				if objects_states.item.item (1).is_equal (a_type_name) then
					Result := True
...
feature -- Compare
	...
	compare (a_state: like Current): DS_ARRAYED_LIST [like state] is
		...
		do
			...
			loop
				if a_state.found_type (objects_states.item.item (1).out) then
					l_count_change := a_state.item_found_count - objects_states.item.integer_item (2)
					if l_count_change /= 0 then
						Result.force_last ([(objects_states.item [1]).out, l_count_change])
					end
				else
					Result.force_last ([(objects_states.item [1]).out, - objects_states.item.integer_32_item (2)])
				end
				objects_states.forth
			end
		ensure
