...
int sglib___##type##_delete_recursive(type **tree, type *elem) {\
  ...
  res = 0;\
  if (t==NULL) {\
    assert(0 && "The element to delete not found in the tree,  use 'delete_if_member'");\
  } else {\
    cmp = comparator(elem, t);\
    if (cmp < 0 || (cmp==0 && elem<t)) {\
      ...
      assert(elem==t && "Deleting an element which is non member of the tree, use 'delete_if_member'");\
      if (t->left == NULL) {\
        if (t->right == NULL) {\
          /* a leaf, delete, it; */\
          *tree = NULL;\
