note
	description: "[
		Arguments for ECF_REMOVE_SAFE objects
	]"
...
class
	ECF_REMOVE_SAFE_ARGUMENT_PARSER
...
feature -- Access
	...
	safe_redirected: BOOLEAN
			-- Indicates if removed -safe.ecf should be kept as a redirection.
		require
			is_successful: is_successful
		once
			Result := has_option (redirect_safe_switch)
		end
...
feature {NONE} -- Switches
	...
	switches: ARRAYED_LIST [ARGUMENT_SWITCH]
		...
		once
			...
			Result.extend (create {ARGUMENT_SWITCH}.make (redirect_safe_switch, "Use redirection to keep the -safe.ecf file", True, False))
			Result.extend (create {ARGUMENT_SWITCH}.make (verbose_switch, "Verbose output", True, False))
		end

	recursive_switch: STRING = "r|recursive"

	redirect_safe_switch: STRING = "redirect_safe"
