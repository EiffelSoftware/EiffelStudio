		...
		do
			...
			from l_files.start until l_files.after loop
				...
				if not l_reg.is_empty then
					working_file := l_reg
					l_reg.keys.do_if (agent generate_registry_key (?, Result.root_element, a_options), agent (a_item: REG_FILE_KEY): BOOLEAN do Result := not a_item.is_empty end)
				end
				l_files.forth
			end
...
feature {NONE} -- Generation
	...
	generate_registry_key (a_key: REG_FILE_KEY; a_parent: XM_ELEMENT; a_options: IREG2WIX_OPTIONS) is
		...
		do
			...
			l_elm.add_attribute (id_attribute_name, default_namespace, generate_new_id)
	...
	generate_registry_value (a_value: REG_FILE_VALUE; a_parent: XM_ELEMENT; a_options: IREG2WIX_OPTIONS) is
		...
		do
			...
			if l_named_value = Void or else l_named_value.is_supported then
				...
				l_elm.add_attribute (value_attribute_name, default_namespace, a_value.value)
