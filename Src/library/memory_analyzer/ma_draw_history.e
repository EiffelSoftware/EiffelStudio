...
feature {NONE} -- Implemention
	...
	draw_history_graph_line (a_history: like graph_used_history; a_color: EV_COLOR; a_width: INTEGER) is
		...
		do
			...
			until
				a_history.after
			loop
				current_x := a_history.item.integer_32_item (1)
				current_percent := a_history.item.double_item (2)	
				if a_history.index >= 2 then
					...
					if last_used_percent /= current_percent or a_history.index /= a_history.count then
						...
						last_used_percent := a_history.item.double_item (2)
						last_used_x := a_history.item.integer_32_item (1)
					end
				else
					last_used_percent := a_history.item.double_item (2)
					last_used_x := a_history.item.integer_32_item (1)
	...
	save_history (used_percent, overhead_percent:DOUBLE)is
		...
		local 
			temp : TUPLE [INTEGER, DOUBLE]
		do
			current_position_x := current_position_x + step
			create temp.default_create
			temp.put_integer_32 (current_position_x	,1)
			temp.put_double (used_percent, 2)
			graph_used_history.extend (temp)
			
			create temp.default_create
			temp.put_integer_32 (current_position_x, 1)
			temp.put_double (overhead_percent, 2)
			graph_overhead_history.extend (temp)
		ensure
...
feature {NONE} -- Fields
	...
	graph_used_history: ARRAYED_LIST [TUPLE [INTEGER, DOUBLE]]
			-- The list store used graph history.
			
	graph_overhead_history :  ARRAYED_LIST [TUPLE [INTEGER, DOUBLE]]
