...
feature {CODE_CONSUMER_FACTORY} -- Visitor features.
	...
	generate_event (a_source: SYSTEM_DLL_CODE_MEMBER_EVENT) is
		...
		do
			a_event_manager.raise_event (feature {CODE_EVENTS_IDS}.Not_implemented, ["member event"])
		end		
	...
	generate_snippet_feature (a_source: SYSTEM_DLL_CODE_SNIPPET_TYPE_MEMBER) is
		...
		require
			non_void_source: a_source /= Void
		local
			a_snippet_feature: CODE_SNIPPET_FEATURE
		do
...
feature {NONE} -- Implementation
	...
	initialize_attribute (a_source: SYSTEM_DLL_CODE_MEMBER_FIELD; an_attribute: CODE_ATTRIBUTE) is
	...
	initialize_snippet_feature (a_source: SYSTEM_DLL_CODE_SNIPPET_TYPE_MEMBER; a_snippet_feature: CODE_SNIPPET_FEATURE) is
...
feature {NONE} -- Components initialization.
	...
	generate_feature_clause (a_source: SYSTEM_DLL_CODE_TYPE_MEMBER; a_feature: CODE_FEATURE) is
				...
				if l_property_getter /= Void then
					...
					if l_property_setter /= Void then
						...
						if l_routine /= Void then
							...
							-- Attribute
							l_eiffel_name := current_type.add_redefine_clause_from_dotnet (a_feature.name, create {LINKED_LIST [CODE_PARAMETER_DECLARATION_EXPRESSION]}.make)
						end
					end
				end
				check
	...
	generate_comments (a_source: SYSTEM_DLL_CODE_TYPE_MEMBER; a_feature: CODE_FEATURE) is
	...
	initialize_feature_clauses (private_implementation_type: SYSTEM_DLL_CODE_TYPE_REFERENCE; a_feature: CODE_ROUTINE) is
	...
	initialize_member_status (status_attributes: SYSTEM_DLL_MEMBER_ATTRIBUTES; a_feature: CODE_FEATURE) is
			-- Initialize.
		require
		...
		do
			...
			if scope_status = feature {SYSTEM_DLL_MEMBER_ATTRIBUTES}.override then
				...
				elseif a_feature.name = Void or else a_feature.name.is_empty then
					Event_manager.raise_event (feature {CODE_EVENTS_IDS}.missing_feature_name, ["member status initialization"])
				else
	...
	initialize_custom_attributes (custom_attributes: SYSTEM_DLL_CODE_ATTRIBUTE_DECLARATION_COLLECTION; a_feature: CODE_FEATURE) is
			-- Initialize `custom_attributes' of `a_feature'.
		require
			non_void_feature: a_feature /= Void
		local
