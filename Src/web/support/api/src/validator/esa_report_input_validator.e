note
	description: "[
					Provides input validations for general reports and user reports
					Example:
					?page=1&size=5&category=0&orderBy=submissionDate&dir=DESC&status=0&status=1&status=2&status=3&status=4&filter=&filter_content=0
				]"
	date: "$Date$"
	revision: "$Revision$"
...
feature -- Initialization
	...
	default_create
			...
			do
				to_implement ("Extract constants!!!")
				set_page (1)
				set_size (30)
				set_category (0)
				default_status
				...
				page_set: page = 1
				size_set: size = 30
				orderby_set: orderby.same_string ("submissionDate")
	...
	validate_status (a_current_keys: ARRAY [READABLE_STRING_GENERAL]; a_table_request: STRING_TABLE [WSF_VALUE])
		...
		do
			if a_current_keys.has ({STRING_32}"status") then
				if
					attached {WSF_MULTIPLE_STRING} a_table_request.at ("status") as l_status
				then
					status.wipe_out
