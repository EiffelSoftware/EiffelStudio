note
	description: "Summary description for {CMS_WISH_CATEGORY_HANDLER}."
	date: "$Date$"
	revision: "$Revision$"

class
	CMS_WISH_CATEGORY_HANDLER
...
feature -- HTTP Methods
	...
	do_get (req: WSF_REQUEST; res: WSF_RESPONSE)
		...
		local
			...
			l_uid: INTEGER_64
			edit_response: CMS_WISH_CATEGORY_FORM_RESPONSE
			view_response: CMS_WISH_CATEGORY_VIEW_RESPONSE
			r: CMS_RESPONSE
		do
			create {FORBIDDEN_ERROR_CMS_RESPONSE} r.make (req, res, api)
			if r.has_permission ("admin wish category") then
				if req.percent_encoded_path_info.ends_with_general ("/edit") then
					check valid_url: req.percent_encoded_path_info.starts_with_general ("/resources/wish/category/") end
					create edit_response.make (req, res, api, wish_api)
				...
				elseif req.percent_encoded_path_info.ends_with_general ("/delete")  then
					...
					if l_uid > 0 then
						l_category := wish_api.category_by_id (l_uid)
						if
							l_category /= Void
						then
							create view_response.make (req, res, api, wish_api)
...
feature {NONE} -- New Category
	...
	create_new_category (req: WSF_REQUEST; res: WSF_RESPONSE)
		local
			edit_response: CMS_WISH_CATEGORY_FORM_RESPONSE
		do
			if req.percent_encoded_path_info.starts_with ("/resources/wish/add/category") then
				create edit_response.make (req, res, api, wish_api)
				edit_response.execute
			else
