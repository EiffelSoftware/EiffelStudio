...
note
	description: "Store a version number as major,minor,build number"
	...
	set_version (vers: READABLE_STRING_GENERAL)
		...
		do
			...
			if l_new_pos > 0 then
				...
				minor := vers.substring (l_pos, vers.count).to_integer
				build := 0
			end
		end
	...
	is_version_valid (vers: READABLE_STRING_GENERAL): BOOLEAN
		...
		do
			if vers /= Void then
				...
				if Result then
						-- It is a valid version number if there was 1 or 2 colons
						-- and that we were waiting for a colon or a digit for
						-- the last input.
					Result := (nb_colons = 1 or nb_colons = 2) and then state = 2
				end
			end
		end

invariant
	positive_composants: major >= 0 and minor >= 0 and build >= 0
