...
--					final_position := l_rect.bottom - zone.height
	...
	handle_timer (a_timer: EV_TIMEOUT) is
		...
		do
			...
			when {SD_SHARED}.dock_right then
			
			when {SD_SHARED}.dock_top then
				
			when {SD_SHARED}.dock_bottom then
	...
	stick_window (a_direction: INTEGER) is
		...
		do
			...
			-- Change the state.
			if internal_direction = {SD_SHARED}.dock_left or a_direction = {SD_SHARED}.dock_right then
