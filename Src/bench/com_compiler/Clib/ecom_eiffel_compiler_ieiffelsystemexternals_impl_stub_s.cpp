...
-----------------------------------------------------------*/
{
	...
	if (tmp_value != NULL)
	{
		...
		*return_value = grt_ec_Eif_compiler.ccom_ec_pointed_interface_164 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
		*return_value = NULL;
...
-----------------------------------------------------------*/
{
	...
	{
		EIF_OBJECT tmp_object = eif_protect (tmp_value);
		*return_value = grt_ec_Eif_compiler.ccom_ec_pointed_interface_167 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
		*return_value = NULL;
