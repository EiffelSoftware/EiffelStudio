...
feature -- Access
	...
	eiffel_feature_name_from_dynamic_args (a_dotnet_type: TYPE; a_dotnet_feature_name: STRING; feature_arguments: LINKED_LIST [ECDP_EXPRESSION]): STRING is
		...
		do
			if Resolver.is_generated_type (a_dotnet_type.name) then
				Result := (create {ECDP_ENTITY_NAME_RESOLVER}).eiffel_entity_name (a_dotnet_feature_name)
			else
			...
			if Result = Void then
				(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Missing_feature, [a_dotnet_type.to_string, a_dotnet_feature_name])
	...
	declaring_type (a_dotnet_type_name: STRING; a_dotnet_feature_name: STRING; feature_arguments: LINKED_LIST [ECDP_EXPRESSION]): STRING is
		...
		do
			...
			if Result = Void then
				(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Non_generated_type, [a_dotnet_type_name, "declaring_type", "feature finder"])
			...
			if not l_rescued then
				if Resolver.is_generated_type (a_dotnet_type.name) then
					...
					if l_cache_reflection.is_assembly_in_cache (a_dotnet_type.assembly.get_name) then
						...
						(create {ECDP_EVENT_MANAGER}).raise_event (feature {ECDP_EVENTS_IDS}.Missing_consumed_assembly, [a_dotnet_type.assembly.get_name])
					end
				end
			end
		rescue
