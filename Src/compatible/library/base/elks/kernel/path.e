...
feature -- Status report
	...
	has_extension (a_ext: READABLE_STRING_GENERAL): BOOLEAN
		...
		require
			...
			a_ext_has_no_dot: not a_ext.has_code (('.').natural_32_code)
		do
			Result := attached extension as l_ext and then l_ext.to_string_32.is_case_insensitive_equal (a_ext.to_string_32)
		end
...
feature -- Access
	...
	absolute_path: PATH
		...
		do
			Result := absolute_path_in (create {PATH}.make_from_string (env.current_working_directory))
		end
...
feature -- Status setting
	...
	appended_with_extension (a_ext: READABLE_STRING_GENERAL): PATH
		...
		require
			...
			a_ext_not_empty: not a_ext.is_empty
			a_ext_has_no_dot: not a_ext.has_code (('.').natural_32_code)
			a_ext_has_no_directory_separator: not a_ext.has_code (windows_separator.natural_32_code) and not a_ext.has_code (unix_separator.natural_32_code)
		...
		do
			if not other.is_empty then
				...
				if a_separator /= '%U' and not a_storage.is_empty then
					...
					if not is_character (a_storage, a_storage.count - unit_size + 1, a_separator) and other.code (1) /= unix_separator.natural_32_code then
						if {PLATFORM}.is_windows then
							if other.code (1) /= windows_separator.natural_32_code then
