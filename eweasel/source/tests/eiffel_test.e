...
feature -- Creation
	...
	make (insts: LIST [TEST_INSTRUCTION]) is
...
feature {TEST_INSTRUCTION} -- Set test properties
	...
	set_e_compilation (e: EIFFEL_COMPILATION) is
		do
			e_compilation := e;
		end;
	...
	set_c_compilation (c: C_COMPILATION) is
		do
			c_compilation := c;
		end;

	set_e_compilation_result (e: EIFFEL_COMPILATION_RESULT) is
		do
			e_compilation_result := e;
		end;

	set_c_compilation_result (c: C_COMPILATION_RESULT) is
		do
			c_compilation_result := c;
		end;

	set_execution_result (e: EXECUTION_RESULT) is
		do
			execution_result := e;
		end;

	set_instructions (l: LIST [TEST_INSTRUCTION]) is
		do
			instructions := l;
		end;
...
feature {TEST_INSTRUCTION}  -- Test properties

	instructions: LIST [TEST_INSTRUCTION];
