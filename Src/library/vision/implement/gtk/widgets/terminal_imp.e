indexing
	...
	description: 
		"EiffelVision terminal, gtk implementation.";
...
feature {NONE} -- Implementation
	...
	children_list: LIST [WIDGET_IMP] is
		...
		do
			...
			if font_implementation.is_valid then
				...
				if a_font_list.is_valid then
					...
					from
--XX						resource := XmNfontList;
						list.start
					until
						list.after
					loop
--XX						set_xm_font_list (list.item, resource, a_font_list);
						list.forth
					end
					a_font_list.destroy
				else
	...
	button_widget_list: LINKED_LIST [POINTER] is
		...
		do
			...
			loop
--				w := list.item;
--XX 				if xm_is_push_button_gadget (w) or else
...
-- 				list.forth
	...
	text_widget_list: LINKED_LIST [POINTER] is
		...
		do
			...
			loop
--				w := list.item;
--XX 				if xm_is_text_field (w) or else
-- 					xm_is_list (w)
-- 				then	
-- 					Result.put_front (w)
-- 				end;
