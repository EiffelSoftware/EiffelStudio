...
feature -- Implementation
	...
	parse (code_stream: TEXT_READER): SYSTEM_DLL_CODE_COMPILE_UNIT is
		...
		require else
			referenced_assemblies_initialized: (create {ECDP_REFERENCED_ASSEMBLIES}).assemblies_initialized
		local
			l_eiffel_parser: EIFFEL_PARSER
			l_class_to_parse: STRING
			l_class_as: CLASS_AS
			l_visitor: ECDP_CODEDOM_VISITOR
			l_support: ECDP_SUPPORT
			l_retried: BOOLEAN
	...
	initialize_attributes (a_class_as: CLASS_AS) is
		...
		local
			l_init_attributes: ECDP_INIT_ATTRIBUTES
		do
...
--			l_code_dom_path: ECDP_CODE_DOM_PATH
		do
...
end -- class EIFFEL_CODE_DOM_PARSER
