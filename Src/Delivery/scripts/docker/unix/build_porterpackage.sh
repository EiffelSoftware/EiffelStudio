...
if [ "$SVN_ISE_REPO" = "" ]; then
	export SVN_ISE_REPO=svn://svn.ise/ise_svn
fi
if [ "$SVN_EIFFELSTUDIO_REPO" = "" ]; then
        export SVN_EIFFELSTUDIO_REPO=svn://svn.ise/mirrors/eiffelstudio
fi
...
docker run --rm \
        ...
        -e SVN_EIFFELSTUDIO_REPO=$ISE_EIFFELSTUDIO_REPO \
