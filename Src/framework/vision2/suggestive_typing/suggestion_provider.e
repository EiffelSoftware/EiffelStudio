...
feature -- Access
	...
	query (a_expression: READABLE_STRING_GENERAL): ARRAYED_LIST [SUGGESTION_ITEM]
			-- Given a query `a_expression', provides a list of possible associated items.
		require
			is_available: is_available
		do
			Result := query_with_cancellation (a_expression, Void)
		end

	query_with_cancellation (a_expression: READABLE_STRING_GENERAL; a_cancel_request: detachable PREDICATE [ANY, TUPLE]): ARRAYED_LIST [SUGGESTION_ITEM]
		...
		do
			...
			query_with_callback_and_cancellation (a_expression, agent (a_list: like query; a_suggestion: SUGGESTION_ITEM)
				do
