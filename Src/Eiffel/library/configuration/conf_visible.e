...
feature {CONF_ACCESS} -- Update, stored to configuration file
	...
	add_visible (a_class, a_feature, a_class_rename, a_feature_rename: STRING) is
		...
		require
			...
			a_class_ok: a_class.is_equal (a_class.as_upper)
			a_feature_ok: a_feature /= Void implies a_feature.is_equal (a_feature.as_lower) and not a_feature.is_empty
			a_class_reanme_ok: a_class_rename /= Void implies a_class_rename.is_equal (a_class_rename.as_upper) and not a_class_rename.is_empty
			a_feature_rename_ok: a_feature_rename /= Void implies a_feature_rename.is_equal (a_feature_rename.as_lower) and not a_feature_rename.is_empty
		...
		do
			...
			if a_feature_rename /= Void then
				l_feature_name := a_feature_rename
			else
			...
			if a_feature /= Void then
				...
				l_v_cl.force (l_feature_name, a_feature)
			end
