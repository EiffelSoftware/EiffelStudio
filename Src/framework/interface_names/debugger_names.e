...
feature -- warnings
	...
	w_Cannot_launch_system: STRING_32 is
		do Result := locale.translation ("Could not launch system.") end

	w_Cannot_find_valid_ecdbgd (a_ecdbgd_path, a_env_var_str, a_env_var_name: STRING_GENERAL): STRING_32 is
		do
	...
	w_Cannot_launch_in_allotted_time (a_timeout: INTEGER_32; a_env_var_str, a_env_var_name: STRING_GENERAL): STRING_32 is
		do
			Result := locale.formatted_string (locale.translation (
...
feature -- Expression evaluation messages
	...
	msg_error_call_on_void_target (fname: STRING_GENERAL): STRING_32 is
		require
			fname /= Void
		do Result := locale.formatted_string (locale.translation ("Error: Call on void target for `$1'."), [fname]) end

	msg_error_vst1_on_class_context (clname, fname: STRING_GENERAL): STRING_32 is
		require
	...
	msg_error_during_context_preparation (s: STRING_GENERAL): STRING_32 is
		require
			s_not_void: s /= Void
	...
	msg_error_not_yet_ready_for (a: ANY; s, f: STRING_GENERAL): STRING_32 is
		require
			a_not_void: a /= Void
			s_not_void: s /= Void
			f_not_void: f /= Void
	...
	msg_error_unknown_constant_type_for (fname: STRING_GENERAL): STRING_32 is
		require
			fname_not_void: fname /= Void
		do Result := locale.formatted_string (locale.translation ("Unknown constant type for `$1'"), [fname]) end

	msg_error_during_evaluation_of_call (a: ANY; fname: STRING_GENERAL): STRING_32 is
		require
			a_not_void: a /= Void
			fname_not_void: fname /= Void
	...
	msg_error_unable_to_get_valid_target_for (add: STRING_GENERAL): STRING_32 is
		require
	...
	msg_error_unable_to_evaluate_call (cname,fname: STRING_GENERAL; addr: STRING_GENERAL; desc: STRING_GENERAL): STRING_32 is
		require
	...
	msg_error_evaluation_wrong_nb_of_args (fnb,pnb: INTEGER): STRING_32 is
		do
			Result := locale.formatted_string (locale.translation ("Wrong number of argument: $2 instead of $1 ."), [fnb, pnb])
		end

	msg_error_evaluation_aborted (cname,fname: STRING_GENERAL): STRING_32 is
		require
			cname_not_void: cname /= Void
			fname_not_void: fname /= Void
		do Result := locale.formatted_string (locale.translation ("Evaluation aborted: {$1}.$2"), [cname, fname]) end

	msg_error_unable_to_evaluate_once_call (cname,fname: STRING_GENERAL): STRING_32 is
		require
			cname_not_void: cname /= Void
			fname_not_void: fname /= Void
		do Result := locale.formatted_string (locale.translation ("Unable to evaluate once routine {$1}.$2"), [cname, fname]) end

	msg_error_unable_to_evaluate_non_once_call_with_any_object (cname,fname: STRING_GENERAL): STRING_32 is
		require
			cname_not_void: cname /= Void
			fname_not_void: fname /= Void
	...
	msg_error_exception_occurred_during_evaluation (cname, fname: STRING_GENERAL; a_trace: STRING_GENERAL): STRING_32 is
		require
			cname_not_void: cname /= Void
			fname_not_void: fname /= Void
		local
	...
	msg_error_once_evaluation_failed (fname: STRING_GENERAL; msg: STRING_GENERAL): STRING_32 is
		require
			fname_not_void: fname /= Void
		do
			if msg = Void then
	...
	msg_error_once_routine_not_yet_called (fname: STRING_GENERAL): STRING_32 is
		require
			fname_not_void: fname /= Void
		do
	...
	msg_error_once_procedure_evaluation_not_yet_available (fname: STRING_GENERAL): STRING_32 is
		require
			fname_not_void: fname /= Void
		do
	...
	msg_error_cannot_find_context_object (addr: STRING_GENERAL): STRING_32 is
		require
	...
	msg_error_type_not_compiled (tname: STRING_GENERAL): STRING_32 is
		require
			tname_not_void: tname /= Void
