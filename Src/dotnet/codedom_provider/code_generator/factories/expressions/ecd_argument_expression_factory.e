...
class
	ECD_ARGUMENT_EXPRESSION_FACTORY
...
feature {NONE} -- Implementation
	...
	initialize_argument_reference_expression (a_source: SYSTEM_DLL_CODE_ARGUMENT_REFERENCE_EXPRESSION; an_argument: ECD_ARGUMENT_REFERENCE_EXPRESSION) is
			-- | Set `name'
			-- Generate Eiffel code from `a_source'.
		require
			non_void_source: a_source /= Void
	...
	initialize_parameter_declaration_expression (a_source: SYSTEM_DLL_CODE_PARAMETER_DECLARATION_EXPRESSION; a_parameter: ECD_PARAMETER_DECLARATION_EXPRESSION) is
			-- | Set`name'
			-- | Set `type'
			-- | Add parameter to 
...
end -- class ECD_ARGUMENT_EXPRESSION_FACTORY
