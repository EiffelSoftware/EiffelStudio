...
--<			Result := {ISE_RUNTIME}.generic_parameter_count (dynamic_type)
			Result := object.generating_type.generic_parameter_count
		end
...
--<			Result := {ISE_RUNTIME}.field_type_of_type (i, dynamic_type)
		...
		do
			l_field_type := object.generating_type.field_static_type (i)
...
--<			Result := {ISE_RUNTIME}.field_static_type_of_type (i, dynamic_type)
		local
			l_type: detachable TYPE [detachable ANY]
		do
			Result := object.generating_type.field_static_type (i).type_id
		ensure
			field_type_nonnegative: Result >= 0
		end
