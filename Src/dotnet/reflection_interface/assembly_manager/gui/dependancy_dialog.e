...
feature {NONE} -- Implementation
	...
	display_dependancies is
		...
		do
			...
			loop
				a_descriptor ?= dependancies_list.Item (i)
				if a_descriptor /= Void then
	...
	fill_data_grid is
		...
		do
			if not retried then
				...
				if (rows.count + 3) * dictionary.Row_height < dictionary.Window_height - 4 * dictionary.Margin - 4 * dictionary.Label_height then
					difference := dictionary.Window_height - 4 * dictionary.Margin - 4 * dictionary.Label_height - (rows.count + 3) * dictionary.Row_height
