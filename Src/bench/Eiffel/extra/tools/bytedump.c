...

{
    ...
    if (bbool ())
    {
        ...
        (void) bbool ();        /* Skip the 'done' flag */

        fprintf (ofp,"Body Id      : %ld\n", blong ());

        /* Skip reserved space for 'Result' - if any */
    ...
    advance ();    /* Consume the 'BC_NO_CLONE_ARGS' */
    NEWL;
