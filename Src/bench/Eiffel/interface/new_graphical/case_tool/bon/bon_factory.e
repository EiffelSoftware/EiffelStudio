indexing
	description: "Objects that builds bon views for a given eiffel model"
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature -- Basic operations
	...
	model_from_xml (node: XM_ELEMENT): EG_ITEM is
		...
		do
			node_name := node.name
			if node_name.is_equal ("BON_CLASS_FIGURE") then
			...
			elseif node_name.is_equal ("BON_CLUSTER_FIGURE") then
			...
			elseif node_name.is_equal ("BON_CLIENT_SUPPLIER_FIGURE") or else node_name.is_equal ("BON_INHERITANCE_FIGURE") then
								...
								if source_cluster /= Void and target_cluster /= Void then
									...
									if source_found /= Void and target_found /= Void then
										...
										if source /= Void and then target /= Void then
											if node_name.is_equal ("BON_CLIENT_SUPPLIER_FIGURE") then
...
feature {NONE} -- Implementation
	...
	new_inheritance_figure (eih: ES_INHERITANCE_LINK): BON_INHERITANCE_FIGURE is
	...
	new_client_supplier_figure (ecs: ES_CLIENT_SUPPLIER_LINK): BON_CLIENT_SUPPLIER_FIGURE is
