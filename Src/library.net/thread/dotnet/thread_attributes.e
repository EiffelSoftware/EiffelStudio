...
feature -- Externals
	...
	default_priority: INTEGER is
		...
		do
			Result := feature {SYSTEM_THREAD_PRIORITY}.normal.to_integer
		end

	min_priority: INTEGER is
			-- Get minimum thread priority for the current architecture.
		do
			Result := feature {SYSTEM_THREAD_PRIORITY}.lowest.to_integer
		end

	max_priority: INTEGER is
			-- Get maximum thread priority for the current architecture.
		do
			Result := feature {SYSTEM_THREAD_PRIORITY}.highest.to_integer
		end
