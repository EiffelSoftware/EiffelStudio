...
feature -- Comparison
	...
	is_less alias "<" (other: INTEGER_64): BOOLEAN is
...
feature -- Basic operations
	...
	plus alias "+" (other: INTEGER_64): INTEGER_64 is
			-- Sum with `other'
		do
			Result := Precursor (other)
		end

	minus alias "-" (other: INTEGER_64): INTEGER_64 is
			-- Result of subtracting `other'
		do
			Result := Precursor (other)
		end

	product alias "*" (other: INTEGER_64): INTEGER_64 is
			-- Product by `other'
		do
			Result := Precursor (other)
		end

	quotient alias "/" (other: INTEGER_64): REAL_64 is
			-- Division by `other'
		do
			Result := Precursor (other)
		end

	identity alias "+": INTEGER_64 is
			-- Unary plus
		do
			Result := Precursor
		end

	opposite alias "-": INTEGER_64 is
			-- Unary minus
		do
			Result := Precursor
		end

	integer_quotient alias "//" (other: INTEGER_64): INTEGER_64 is
			-- Integer division of Current by `other'
		do
			Result := Precursor (other)
		end

	integer_remainder alias "\\" (other: INTEGER_64): INTEGER_64 is
