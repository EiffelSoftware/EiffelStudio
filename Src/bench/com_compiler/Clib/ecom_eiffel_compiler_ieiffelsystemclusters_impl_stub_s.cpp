...
-----------------------------------------------------------*/
{
	...
	if (tmp_value != NULL)
	{
		...
		*return_value = grt_ec_ISE.ccom_ec_pointed_interface_179 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
		*return_value = NULL;
...
-----------------------------------------------------------*/
{
	...
	{
		EIF_OBJECT tmp_object = eif_protect (tmp_value);
		*return_value = grt_ec_ISE.ccom_ec_pointed_interface_182 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
		*return_value = NULL;
