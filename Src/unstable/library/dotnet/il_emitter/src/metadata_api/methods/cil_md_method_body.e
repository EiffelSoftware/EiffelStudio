...
feature -- Access
	...
	exception_block: CIL_MD_EXCEPTION_CATCH
			-- Exception clause for current feature.

	once_catch_block: CIL_MD_EXCEPTION_CATCH
			-- Exception clause for catch wrapper of once feature.

	once_finally_block: CIL_MD_EXCEPTION_FINALLY
			-- Exception clause for finally wrapper of once feature.

	old_exception_catch_blocks: detachable ARRAY [CIL_MD_EXCEPTION_CATCH]
			-- Exception clause for catching exception for old expression evaluation.

	current_old_exception_catch_block: detachable CIL_MD_EXCEPTION_CATCH
...
feature -- Settings
	...
	create_old_exception_catch_blocks (a_count: INTEGER)
		...
		local
			l_block: CIL_MD_EXCEPTION_CATCH
			i: INTEGER
...
feature -- Opcode insertion
	...
	put_nop
		...
		require
			not_yet_written: not is_written
		do
			internal_put ({CIL_MD_OPCODES}.nop.to_integer_8, current_position)
	...
	put_throw
		...
		require
			not_yet_written: not is_written
		do
			put_opcode ({CIL_MD_OPCODES}.throw)
	...
	put_rethrow
		...
		require
			not_yet_written: not is_written
		do
			put_opcode ({CIL_MD_OPCODES}.rethrow)
			last_current_stack_depth := -1
		end
	...
	put_string (string_token: INTEGER)
		...
		require
			not_yet_written: not is_written
		do
			put_opcode ({CIL_MD_OPCODES}.Ldstr)
...
feature -- Opcode insertion with manual update of `current_stack_depth'.
	...
	put_static_call (feature_token, nb_arguments: INTEGER; is_function: BOOLEAN)
		...
		require
			not_yet_written: not is_written
		do
			internal_put_call ({CIL_MD_OPCODES}.call, feature_token,
	...
	put_newobj (constructor_token: INTEGER; nb_arguments: INTEGER)
		...
		require
			not_yet_written: not is_written
		do
			internal_put_call ({CIL_MD_OPCODES}.newobj, constructor_token,
