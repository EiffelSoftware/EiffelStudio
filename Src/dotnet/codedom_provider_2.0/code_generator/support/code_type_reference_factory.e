	...
	type_reference_name (a_type_reference: SYSTEM_DLL_CODE_TYPE_REFERENCE): STRING is
		...
		do
			if a_type_reference.array_element_type /= Void then
				...
				-- Add current namespace if non specified, this is required because the codedom
				-- generated by ASP.NET may not specify the namespace if the type is in the current
				-- namespace (generated).
				if not Result.has ('.') and not current_namespace.name.is_empty then
					Result.prepend (current_namespace.name + ".")
				end
			end
		end
