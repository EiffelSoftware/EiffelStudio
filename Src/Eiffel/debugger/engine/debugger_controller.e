		...
		do
			...
			elseif (not manager.application_is_executing) then
				...
				then
					...
					uf := u.make_raw_file (Eiffel_system.application_name (True))
					make_f := u.make_text_file_in (Makefile_SH, project_location.workbench_path)
								...
								if not manager.execution_ignoring_breakpoints then
									...
									if l_il_env.use_cordbg (dotnet_debugger) then
										...
										(create {COMMAND_EXECUTOR}).execute_with_args
											(l_app_string,
												safe_path (eiffel_system.application_name (True).to_string_32) + " " + param.arguments)
										launch_program := True
									elseif l_il_env.use_dbgclr (dotnet_debugger) then
											-- Launch DbgCLR.exe.
										(create {COMMAND_EXECUTOR}).execute_with_args
											(l_app_string,
												safe_path (eiffel_system.application_name (True).to_string_32))
										launch_program := True
									end
								else
	...
	start_workbench_application (param: DEBUGGER_EXECUTION_RESOLVED_PROFILE)
		...
		do
			...
			then
				...
				create f.make_with_name (appl_name)
				if not f.exists then
					...
					if Eiffel_system.System.il_generation then
						...
						create make_f.make_with_path ((create {PATH}.make_from_string (project_location.workbench_path.to_string_32)).extended (Makefile_SH))
					...
					if launch_it then
						...
						env32.force (project_location.workbench_path, {STRING_32} "MELT_PATH")
			...
			then
				...
				if not f.exists then
					...
					if Eiffel_system.System.il_generation then
						...
						make_f := u.make_raw_file_in (Makefile_SH, project_location.final_path)
						if make_f.exists and then make_f.date > f.date then
							warning (Warning_messages.w_MakefileSH_more_recent)
						else
							launch_it := True
