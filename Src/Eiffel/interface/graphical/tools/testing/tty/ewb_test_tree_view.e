...
feature {NONE} -- Access
	...
	node_sorter: DS_SORTER [attached TAG_BASED_TREE_NODE [TEST_I]]
			-- Sorter for {TAG_BASED_TREE_NODE}.
		local
			l_cache: like node_sorter_cache
			l_comparator: AGENT_BASED_EQUALITY_TESTER [attached TAG_BASED_TREE_NODE [TEST_I]]
			l_sorter: DS_QUICK_SORTER [attached TAG_BASED_TREE_NODE [TEST_I]]
		do
			l_cache := node_sorter_cache
			if l_cache = Void then
				create l_comparator.make (
					agent (a_node1, a_node2: attached TAG_BASED_TREE_NODE [TEST_I]): BOOLEAN
						do
...
feature {NONE} -- Implementation
	...
	print_node_container (a_node: TAG_BASED_TREE_NODE_CONTAINER [TEST_I]; a_depth: NATURAL)
		...
		local
			l_nodes: DS_ARRAYED_LIST [attached TAG_BASED_TREE_NODE [TEST_I]]
	...
	print_indented_test (a_test: attached TEST_I; a_depth: NATURAL)
