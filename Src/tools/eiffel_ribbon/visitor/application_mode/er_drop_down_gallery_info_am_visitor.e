...
class
	ER_DROP_DOWN_GALLERY_INFO_AM_VISITOR
...
feature -- Command
	...
	visit_drop_down_gallery_recursive (a_node: XML_ELEMENT)
			-- Visit drop down gallery recursively
		local
			l_cmd_id: STRING
			l_vision_nodes: ARRAYED_LIST [EV_TREE_NODE]
			l_data: detachable ER_TREE_NODE_DROP_DOWN_GALLERY_DATA
		do
			if a_node.name.same_string (constants.drop_down_gallery) then
				from
					a_node.start
				until
					a_node.after
				loop
					if attached {XML_ATTRIBUTE} a_node.item_for_iteration as l_atrribute then
						if l_atrribute.name.same_string (constants_attribute.command_name) then
							...
							if attached  l_vision_nodes.first as l_first then
								l_first.wipe_out
								if attached {ER_TREE_NODE_DROP_DOWN_GALLERY_DATA} l_first.data as l_data_tmp then
									l_data := l_data_tmp
								end
							end
						else
					...
					elseif attached {ER_XML_TREE_ELEMENT} a_node.item_for_iteration as l_sub_node and then l_data /= Void then
						if l_sub_node.name.same_string (constants.drop_down_gallery_menu_layout) then
							fill_data_with_drop_down_gallery_sub_node (l_data, l_sub_node)
						end
					end
					a_node.forth
				end
				...
				loop
					if attached {XML_ELEMENT} a_node.item_for_iteration as l_node then
						visit_drop_down_gallery_recursive (l_node)
					end
	...
	fill_data_with_drop_down_gallery_sub_node (a_data: ER_TREE_NODE_DROP_DOWN_GALLERY_DATA; a_drop_down_gallery_menu_layout: ER_XML_TREE_ELEMENT)
			-- Filling data with info from dropdown gallery sub nodes
		require
			not_void: a_data /= Void
			not_void: a_drop_down_gallery_menu_layout /= Void
			valid: a_drop_down_gallery_menu_layout.name.same_string (constants.drop_down_gallery_menu_layout)
		do
			from
