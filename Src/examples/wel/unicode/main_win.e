...
feature {NONE} -- Implementation
	...
	notify (control: WEL_CONTROL; notify_code: INTEGER) is
		...
		do
			...
			elseif control = avail_input_locales_combo then		
				...
				loop
					if avail_input_locales_combo.selected_item = cnt then
	...
	init_locale_list is
		...
		do
			...
			loop
				langid := imm.get_language_identifier (input_locales.item (cnt))
				lang_string := identifiers.item (langid)
				if lang_string = Void then
					avail_input_locales_combo.add_string ("Unknown Language")
				else
					avail_input_locales_combo.add_string (lang_string)
				end
				cnt := cnt + 1
			end
	...
	change_controls (str: STRING) is
		...
		do
			...
			tvitem := tree_view.selected_item
			tvitem.set_text (str)
