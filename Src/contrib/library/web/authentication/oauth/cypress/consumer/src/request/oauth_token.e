...
feature -- Element Change
	...
	set_secret (a_secret: READABLE_STRING_GENERAL)
			-- Set `secret' with `a_secret'
		do
			secret := a_secret.as_string_8
		ensure
			secret_set: secret = a_secret
		end

	set_refresh_token (a_refresh_token: READABLE_STRING_GENERAL)
			-- Set `refresh_token' with `a_refresh_token'
		do
			refresh_token := a_refresh_token.as_string_8
		ensure
	...
	set_raw_response (a_response: READABLE_STRING_GENERAL)
			-- Set `raw_response' with `a_response'
		do
			raw_response := a_response.as_string_8
		ensure
