...
feature -- Access
	...
	display_agent (name: STRING; string_handler: ORDERED_STRING_HANDLER): PROCEDURE [ANY, TUPLE [INTEGER, INTEGER, DOUBLE, DOUBLE, DOUBLE, INTEGER, INTEGER]] is
...
feature {NONE} -- Implementation
	...
	internal_display_agent (x, y: INTEGER; x_tilt, y_tilt, pressure: DOUBLE width, height: INTEGER; name: STRING; string_handler: ORDERED_STRING_HANDLER) is
			-- Display all other arguments of `Current' on `textable', prepended
			-- with `name' fired.
		do
			string_handler.record_string (name + " fired.%Nx : " + x.out + " y : " + y.out +
