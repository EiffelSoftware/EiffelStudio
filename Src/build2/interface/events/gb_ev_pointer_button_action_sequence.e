...
feature -- Access
	...
	display_agent (name: STRING; textable: EV_TEXTABLE): PROCEDURE [ANY, TUPLE [INTEGER, INTEGER, INTEGER, DOUBLE, DOUBLE, DOUBLE, INTEGER, INTEGER]] is
...
feature {NONE} -- Implementation
	...
	internal_display_agent (x, y, button: INTEGER; x_tilt, y_tilt, pressure: DOUBLE width, height: INTEGER; name: STRING; textable: EV_TEXTABLE) is
			-- Display all other arguments of `Current' on `textable', prepended
			-- with `name' fired.
		do
			textable.set_text (name + " fired.%Nx : " + x.out + " y : " + y.out + " button : " + button.out +
