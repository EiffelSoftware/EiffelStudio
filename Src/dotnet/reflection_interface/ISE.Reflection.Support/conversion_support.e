...
feature -- Access
	...
	rename_clause_from_text (a_text: STRING): ISE_REFLECTION_RENAMECLAUSE is 
		...
		do
			...
			Result.settargetname (target_name)
		ensure
			rename_clause_created: Result /= Void
			non_void_source_name: Result.sourcename /= Void
			not_empty_source_name: Result.sourcename.length > 0
			non_void_target_name: Result.targetname /= Void
			not_empty_target_name: Result.targetname.length > 0
		end
	...
	export_clause_from_text (a_text: STRING): ISE_REFLECTION_EXPORTCLAUSE is
		...
		do
			...
			opening_curl_bracket_index := a_text.indexof_string_int32 (Opening_curl_bracket, 0)
			if opening_curl_bracket_index /= -1 then
				closing_curl_bracket_index := a_text.indexof_string_int32 (Closing_curl_bracket, opening_curl_bracket_index)
				if closing_curl_bracket_index /= -1 then
					Result.make (a_text.substring (closing_curl_bracket_index + Closing_curl_bracket.length + 1).trim)
					exportation_string := a_text.substring_int32_int32 (0, closing_curl_bracket_index)	
					if exportation_string.indexof_char (',') = -1 then
						Result.addexportation (exportation_string)	
					else	
						from
						until
							exportation_string.indexof_char (',') = -1 
						loop
							comma_index := exportation_string.indexof_char (',')
							Result.addexportation (exportation_string.substring_int32_int32 (0, comma_index))
