...
class
	REGISTRATION_CLIENT_CHECKED
alias
	"RegistrationClient.ClientChecked"
...
on_ok_event_handler (sender: ANY; arguments: SYSTEM_EVENTARGS) is
	...
	do
		input_checker.check_registrant (title_combo_box.text,
						...
						city_text_box.text,
						state_text_box.text,
						zip_code_text_box.text,
						country_text_box.text)
		if input_checker.success then
			registrar.add_registrant (title_combo_box.text, first_name_text_box.text,
				...
				country_text_box.text)
			input_checker.check_registration (registrar.last_registrant_identifier,
							quantity_up_down.value.tostring,
							discount_plan_combo_box.text)
		end
		if input_checker.success then
			...
			message_result := message_box_factory.show (input_checker.status, "Registration Failed")
		end
	end
