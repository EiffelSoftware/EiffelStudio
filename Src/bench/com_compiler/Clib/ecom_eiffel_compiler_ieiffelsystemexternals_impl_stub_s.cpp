...
-----------------------------------------------------------*/
{
	...
	if (tmp_value != NULL)
	{
		...
		*return_value = grt_ec_ISE.ccom_ec_pointed_interface_193 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
		*return_value = NULL;
...
-----------------------------------------------------------*/
{
	...
	{
		EIF_OBJECT tmp_object = eif_protect (tmp_value);
		*return_value = grt_ec_ISE.ccom_ec_pointed_interface_196 (eif_access (tmp_object));
		eif_wean (tmp_object);
	}
	else
		*return_value = NULL;
