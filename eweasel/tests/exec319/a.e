	...
	make
		...
		do
			...
			report_violation (
				...
				-- Loops in inherited assertions
			report (is_tested_with_assertions)
		end
...
feature {NONE} -- Tests
	...
	is_tested_with_assertions: BOOLEAN
		require
			across data (<<1, -2, -3>>) as c until c.item = -2 all c.item > 0 end
		deferred
		ensure
			across data (<<-1, 2, -3>>) as c until c.item = -3 some c.item > 0 end
		end
