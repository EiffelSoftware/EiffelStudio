...
feature
	...
	generate_regular_assignment (how: INTEGER)
		...
		do
				-- Factory function
--			if
--				attached {CREATION_EXPR_B} source as l_source and then
--				l_source.is_creation_instruction and then attached {FEATURE_B} l_source.call as l_call_b and then
--				l_call_b.is_once
--			then
--				if context.workbench_mode and then attached {FEATURE_BW} l_call_b as l_call_bw then
--					create l_creation
--					l_creation.set_type (source.type)
--					l_creation.set_info (l_source.info)
--					l_creation.set_register (source.register)
--					l_creation.get_register
--					l_creation.set_parameters (l_source.parameters)
--					l_call_bw.set_precursor_type (source.type)
--					l_call_bw.set_type (source.type)
--					l_call_bw.analyze_on (source.register)
--					l_call_bw.set_instance_free_creation (l_creation)
--					l_call_bw.generate
--				end
--			else
				source.generate
