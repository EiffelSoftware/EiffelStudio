indexing
	...
	description:
		"A poll medium used for %
		%polling mediums (multiplexing).";
...
feature 
	...
	execute (total_number,  time_out_sec, time_out_msec: INTEGER) is
		require
			valid_number: total_number > 0;
		local
			number_ready: INTEGER;
		do
			number_ready := medium_select (total_number, time_out_sec, time_out_msec);
	...
	medium_select (number_to_check, time_out_sec, time_out_msec: INTEGER): INTEGER is
		...
		local
			lrm, lwm, lem: ANY
		do
			if not ignore_read and then
			    not read_command_list.all_cleared then
				last_read_mask := clone (read_mask);
				lrm := last_read_mask.mask;
			end;
			if not ignore_write and then
			    not write_command_list.all_cleared then
				last_write_mask := clone (write_mask);
				lwm := last_write_mask.mask;
			end;
			if not ignore_exception and then
			    not exception_command_list.all_cleared then
			...
			if wait then
				...
				Result := c_select (number_to_check, $lrm, $lwm, $lem, time_out_sec, time_out_msec);
			end;
		end;
	...
	process_selected (number_of_selected: INTEGER) is
		...
		do
			if not ignore_read and then
				...
				loop
					if read_command_list.item (counter1) /= Void then
						if last_read_mask.is_medium_ready 
							(read_command_list.item(counter1).active_medium) 
						    then
							read_command_list.item (counter1).execute;
							counter := counter + 1;
						end;
					end;
					counter1 := counter1 + 1;
			...
			if not ignore_write and then
			    counter < number_of_selected and  then 
			    not write_command_list.all_cleared then
				from 
				...
				until
					counter1 > write_command_list.upper or else 
						not (counter < number_of_selected)
				loop
					if write_command_list.item (counter1) /= Void then
						if last_write_mask.is_medium_ready 
							(write_command_list.item(counter1).active_medium) 
						    then
							write_command_list.item (counter1).execute;
							counter := counter + 1;
						end;
					end;
					counter1 := counter1 + 1;
			...
			if not ignore_exception and then 
				...
				until
					counter1 > exception_command_list.upper or else 
						not (counter < number_of_selected)
				loop
					if exception_command_list.item (counter1) /= Void then
						if last_except_mask.is_medium_ready 
							(exception_command_list.item(counter1).active_medium) 
						    then
							exception_command_list.item (counter1).execute;
							counter := counter + 1;
						end;
					end;
						counter1 := counter1 + 1;
