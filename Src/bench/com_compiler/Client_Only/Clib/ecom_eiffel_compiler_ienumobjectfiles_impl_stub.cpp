...
#include "ecom_eiffel_compiler_IEnumObjectFiles_impl_stub.h"
...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_227 (rgelt, NULL));
	}
	...
	grt_ec_ISE_c.ccom_ec_pointed_cell_227 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
...
-----------------------------------------------------------*/
{
	...
	if (ppenum != NULL)
	{
		tmp_ppenum = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_222 (ppenum, NULL));
		if (*ppenum != NULL)
			(*ppenum)->AddRef ();
	}
	...
	if (*ppenum != NULL)
		(*ppenum)->Release ();
	grt_ec_ISE_c.ccom_ec_pointed_cell_222 (((tmp_ppenum != NULL) ? eif_wean (tmp_ppenum) : NULL), ppenum);
...
-----------------------------------------------------------*/
{
	...
	if (rgelt != NULL)
	{
		tmp_rgelt = eif_protect (grt_ce_ISE_c.ccom_ce_pointed_cell_229 (rgelt, NULL));
	}
	...
	if (*rgelt != NULL)
		rt_ce.free_memory_bstr (*rgelt);
	grt_ec_ISE_c.ccom_ec_pointed_cell_229 (((tmp_rgelt != NULL) ? eif_wean (tmp_rgelt) : NULL), rgelt);
...
STDMETHODIMP ecom_eiffel_compiler::IEnumObjectFiles_impl_stub::GetTypeInfo( unsigned int itinfo, LCID lcid, ITypeInfo **pptinfo )
...
STDMETHODIMP ecom_eiffel_compiler::IEnumObjectFiles_impl_stub::GetTypeInfoCount( unsigned int * pctinfo )
...
STDMETHODIMP ecom_eiffel_compiler::IEnumObjectFiles_impl_stub::GetIDsOfNames( REFIID riid, OLECHAR ** rgszNames, unsigned int cNames, LCID lcid, DISPID *rgdispid )
...
STDMETHODIMP ecom_eiffel_compiler::IEnumObjectFiles_impl_stub::Invoke( DISPID dispID, REFIID riid, LCID lcid, unsigned short wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, unsigned int *puArgErr )
