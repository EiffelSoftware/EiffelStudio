...
feature -- Testing
	...
	default_tests: ARRAY [READABLE_STRING_GENERAL]
		once
			...
			Result := {ARRAY [READABLE_STRING_GENERAL]} <<"tk.interface_inheritance","tk.modules","tk.define_entry_point_net6", "tk.define_implementation", "tk.define_interface", "tk.ast_process">>
		end
	...
	test_metadata_tables_token_interface (a_pattern: READABLE_STRING_GENERAL)
		do
			...
			if is_test_included ("define_member_ref", a_pattern) then
				(create {TEST_METADATA_TABLES_TK}).test_define_member_ref;
			end
			if is_test_included ("define_method", a_pattern) then
				(create {TEST_METADATA_TABLES_TK}).test_define_method;
			end
			if is_test_included ("define_field", a_pattern) then
				(create {TEST_METADATA_TABLES_TK}).test_define_field;
			end
...
--			end
			...
			if is_test_included("interface_inheritance", a_pattern) then
				(create {TEST_INTERFACE_INHERITANCE}).test_inheritance
			end
...
--			end
			if is_test_included ("method_assembly", a_pattern) then
				(create {TEST_METADATA_TABLES_OM}).test_define_method;
			end
			if is_test_included ("define_type_ref", a_pattern) then
