indexing
	description: "Object that represents a class generic criterion in Eiffel query lanagage"
...
feature -- Access
	...
	scope: QL_SCOPE is
		...
		do
			...
			good_result: Result.is_equal (generic_scope)
		end

	domain_generator (a_enable_fill_domain: BOOLEAN; a_apply_current: BOOLEAN): QL_GENERIC_DOMAIN_GENERATOR is
...
feature -- Logic operations
	...
	infix "and" (other: QL_GENERIC_CRITERION): QL_GENERIC_CRITERION is
			-- Boolean conjunction with `other'
		do
			create {QL_GENERIC_AND_CRITERION}Result.make (Current, other)
		end

	prefix "not": QL_GENERIC_CRITERION is
			-- Negation
		do
			create {QL_GENERIC_NOT_CRITERION}Result.make (Current)
		end

	infix "or" (other: QL_GENERIC_CRITERION): QL_GENERIC_CRITERION is
			-- Boolean disjunction with `other'		
		do
			create {QL_GENERIC_OR_CRITERION}Result.make (Current, other)
		end
	...
	compiled_criterion: QL_GENERIC_CRITERION is
		...
		do
			if require_compiled then
				create {QL_GENERIC_IS_COMPILED_CRI_IMP}Result.make (Current)
			else
				Result := Current
			end
		end
