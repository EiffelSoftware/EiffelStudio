		...
		do
			...
			if not grid.header.is_empty then
				...
				loop
					...
					if grid.is_row_height_fixed and not grid.is_tree_enabled then
						current_item_y_position := (grid.row_height * (row_counter - 1)) - (internal_client_y - vertical_buffer_offset)
						current_row_height := grid.row_height
					else
						current_item_y_position := (row_offsets @ (row_counter)) - (internal_client_y - vertical_buffer_offset)
						if grid.is_row_height_fixed then
							current_row_height := grid.row_height
						else
