...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->RemoteDraw(tmp_dw_draw_aspect,tmp_lindex,tmp_pv_aspect,ptd,tmp_hdc_target_dev,tmp_hdc_draw,lprc_bounds,lprc_wbounds,p_continue);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IViewObjectEx_impl_proxy::ccom_remote_get_color_set(  /* [in] */ EIF_INTEGER dw_draw_aspect,  /* [in] */ EIF_INTEGER lindex,  /* [in] */ EIF_INTEGER pv_aspect,  /* [in] */ ecom_control_library::tagDVTARGETDEVICE * ptd,  /* [in] */ EIF_INTEGER hic_target_dev,  /* [out] */ EIF_OBJECT pp_color_set )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->RemoteGetColorSet(tmp_dw_draw_aspect,tmp_lindex,tmp_pv_aspect,ptd,tmp_hic_target_dev,tmp_pp_color_set);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IViewObjectEx_impl_proxy::ccom_remote_freeze(  /* [in] */ EIF_INTEGER dw_draw_aspect,  /* [in] */ EIF_INTEGER lindex,  /* [in] */ EIF_INTEGER pv_aspect,  /* [out] */ EIF_OBJECT pdw_freeze )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->RemoteFreeze(tmp_dw_draw_aspect,tmp_lindex,tmp_pv_aspect,tmp_pdw_freeze);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
void ecom_control_library::IViewObjectEx_impl_proxy::ccom_set_advise(  /* [in] */ EIF_INTEGER aspects,  /* [in] */ EIF_INTEGER advf,  /* [in] */ ecom_control_library::IAdviseSink * p_adv_sink )
...
void ecom_control_library::IViewObjectEx_impl_proxy::ccom_remote_get_advise(  /* [out] */ EIF_OBJECT p_aspects,  /* [out] */ EIF_OBJECT p_advf,  /* [out] */ EIF_OBJECT pp_adv_sink )
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->RemoteGetAdvise(tmp_p_aspects,tmp_p_advf,tmp_pp_adv_sink);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->IViewObject2_GetExtent(tmp_dw_draw_aspect,tmp_lindex,ptd,lpsizel);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->GetRect(tmp_dw_aspect,p_rect);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->QueryHitPoint(tmp_dw_aspect,p_rect_bounds,*(ecom_control_library::tagPOINT*)ptl_loc,tmp_l_close_hint,tmp_p_hit_result);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	hr = p_IViewObjectEx->QueryHitRect(tmp_dw_aspect,p_rect_bounds,p_rect_loc,tmp_l_close_hint,tmp_p_hit_result);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
...
-----------------------------------------------------------*/
{
	...
	LONG tmp_lindex = 0;
	tmp_lindex = (LONG)lindex;
	ecom_control_library::wireHDC tmp_hic_target_dev = 0;
	tmp_hic_target_dev = (ecom_control_library::wireHDC)hic_target_dev;
	
	hr = p_IViewObjectEx->GetNaturalExtent(tmp_dw_aspect,tmp_lindex,ptd,tmp_hic_target_dev,p_extent_info,psizel);
	if (FAILED (hr))
	{
		if ((HRESULT_FACILITY (hr)  ==  FACILITY_ITF) && (HRESULT_CODE (hr) > 1024) && (HRESULT_CODE (hr) < 1053))
			com_eraise (rt_ec.ccom_ec_lpstr (eename(HRESULT_CODE (hr) - 1024), NULL),HRESULT_CODE (hr) - 1024);
