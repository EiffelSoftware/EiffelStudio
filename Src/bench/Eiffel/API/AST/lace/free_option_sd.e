...
feature {COMPILER_EXPORTER}
	...
	process_system_level_options (value: OPT_VAL_SD) is
		...
		do
			...
			when profiler_type then
				if value = Void then
					error_found := True

					-- ***** FIXME *****
					-- What about this:
					-- Two weeks ago I had in my ace file: profiler_type(eiffel)
					-- Right now `System.profiler_type' will be "gprof" (because
					-- I didn't trust E_PROF ;-).
					-- Then incidently, I'm trying to convert "profinfo.e".
					-- CONFIGURATION_LOADER will grep `System.profiler_type' and
					-- try to parse "profinfo.e" as if it has been generated by
					-- gprof: aiiieeeeeee!!!!
					-- ***** FIXME *****
