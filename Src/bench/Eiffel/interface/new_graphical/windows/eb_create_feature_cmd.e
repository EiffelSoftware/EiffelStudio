...
--				create wd.make_default (Project_tool.parent, Interface_names.t_Warning, Warning_messages.w_Feature_not_compiled)
--			else
--				create_class_tool (st)
--			end
--		end
...
feature {NONE} -- Implementation
	...
	tool: EB_PROJECT_TOOL
		-- is that accurate?
		-- it could be the selector instead

	create_feature_tool (a_stone: STONE) is
...
--			new_tool.receive (a_stone)
	...
	execute (arg: EV_ARGUMENT1 [ANY]; data: EV_EVENT_DATA) is
		do
			create_feature_tool (Void)
		end

end -- class EB_CREATE_FEATURE_CMD
