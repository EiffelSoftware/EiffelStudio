...
feature -- Initialization
	...
	initialize_env_vars is
		...
		local
			...
			l_path: STRING
		do
			l_path := short_path (eiffel_layout.borland_directory)
			create l_file_name.make_from_string (eiffel_layout.borland_directory)
			...
			if (create {DIRECTORY}.make (l_file_name)).exists then
				...
				if not l_config_file.exists then
					l_arg_string := "-D_WIN32_WINDOWS=0x0410 %
										%-DWINVER=0x400 %
										%-I"+l_path+"\BCC55\include %
										%-L"+l_path+"\BCC55\lib %
										%-L"+l_path+"\BCC55\lib\PSDK"
