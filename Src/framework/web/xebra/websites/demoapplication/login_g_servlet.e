...
feature-- Implementation
	...
	handle_request (request: XH_REQUEST; response: XH_RESPONSE)
		do
				-- row: 1 column: 1 path: ../../websites/demoapplication/login.xeb
			response.append("[

]")
				-- row: 35 column: 29 path: ../../websites/demoapplication/login.xeb
			response.append("[
<html>
<head>
	...
	<tr>
		...
		<div align="right">
			...
			if controller.authenticated then
				-- row: 36 column: 29 path: ../../websites/demoapplication/login.xeb
			response.append("[
       ...
       Welcome 
			...
			response.append(controller.username.out)
				-- row: 37 column: 11 path: ../../websites/demoapplication/login.xeb
			response.append("[
, <a href="logout.xeb">Logout</a>
		
]")
			end
				-- row: 37 column: 20 path: ../../websites/demoapplication/login.xeb
			response.append("[

]")
				-- row: 37 column: 20 path: ../../websites/demoapplication/login.xeb
			if controller.not_authenticated then
				-- row: 39 column: 11 path: ../../websites/demoapplication/login.xeb
			response.append("[
			...
			<a href="login.xeb">Login</a>
				...
				-- row: 70 column: 12 path: ../../websites/demoapplication/login.xeb
			response.append("[
</div>
		</td>
	...
	<tr>
		...
		<h2><span class="style1">Login</span></h2>
				...
				-- row: 70 column: 12 path: ../../websites/demoapplication/login.xeb
			controller.login
				-- row: 70 column: 39 path: ../../websites/demoapplication/login.xeb
			response.append("[

]")
				-- row: 70 column: 39 path: ../../websites/demoapplication/login.xeb
			if controller.authenticated then
				-- row: 72 column: 11 path: ../../websites/demoapplication/login.xeb
			response.append("[
			...
			<p>You have successfully been logged in.</p>
				...
				-- row: 72 column: 20 path: ../../websites/demoapplication/login.xeb
			response.append("[

]")
				-- row: 72 column: 20 path: ../../websites/demoapplication/login.xeb
			if controller.not_authenticated then
				-- row: 92 column: 11 path: ../../websites/demoapplication/login.xeb
			response.append("[
			...
			<table width="300" border="0" cellpadding="3" cellspacing="5" bgcolor="#FFFFFF">
				<tr>
					...
					<td bgcolor="#666666"><label> <input type="text" name="name" id="name"></input> </label></td>
				</tr>
				<tr bgcolor="#333333">
			...
			<p align="center"><input type="submit" name="submit" id="submit" value="Login"></input></p>
			</form>
		
]")
			end
				-- row: 111 column: 8 path: ../../websites/demoapplication/login.xeb
			response.append("[
 </td>
		<td></td>
	</tr>
